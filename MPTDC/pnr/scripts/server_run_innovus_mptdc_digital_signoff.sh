...
apply_recovery_defaults() {
  ...
  export MPTDC_PNR_CREATE_RO_ROUTE_BLOCKAGES="${MPTDC_PNR_CREATE_RO_ROUTE_BLOCKAGES:-0}"
  export MPTDC_RO_ROUTE_BLOCKAGE_MARGIN_UM="${MPTDC_RO_ROUTE_BLOCKAGE_MARGIN_UM:-1.0}"
  export MPTDC_RO_ROUTE_BLOCKAGE_LAYERS="${MPTDC_RO_ROUTE_BLOCKAGE_LAYERS:-MET1 MET2 MET3 METTP}"
  export MPTDC_RO_ROUTE_BLOCKAGE_OPEN_SIDES="${MPTDC_RO_ROUTE_BLOCKAGE_OPEN_SIDES:-north south}"
  ...
  export MPTDC_STOP_AFTER_POSTPLACE_PRE_ROUTE_SROUTE="${MPTDC_STOP_AFTER_POSTPLACE_PRE_ROUTE_SROUTE:-0}"
  ...
  export MPTDC_SROUTE_MODE_PROFILE="${MPTDC_SROUTE_MODE_PROFILE:-}"
...
guard_pg_policy() {
  ...
  case "$pg_strategy" in
    ...
    conservative_ro_hookup)
      if ! is_truthy "${MPTDC_REQUIRE_POSTPLACE_PRE_ROUTE_SROUTE_CLEAN:-1}"; then
        failures+=("MPTDC_REQUIRE_POSTPLACE_PRE_ROUTE_SROUTE_CLEAN=${MPTDC_REQUIRE_POSTPLACE_PRE_ROUTE_SROUTE_CLEAN:-unset} expected 1 for conservative_ro_hookup")
      fi
      if is_truthy "${MPTDC_POSTPLACE_PRE_ROUTE_ALLOW_DANGLING_ONLY:-0}"; then
        failures+=("MPTDC_POSTPLACE_PRE_ROUTE_ALLOW_DANGLING_ONLY=${MPTDC_POSTPLACE_PRE_ROUTE_ALLOW_DANGLING_ONLY:-unset} expected 0 for conservative_ro_hookup")
      fi
      if is_truthy "${MPTDC_ROUTE_GATE_SROUTE_RECOVERY:-0}"; then
        failures+=("MPTDC_ROUTE_GATE_SROUTE_RECOVERY=${MPTDC_ROUTE_GATE_SROUTE_RECOVERY:-unset} expected 0 for conservative_ro_hookup")
      fi
      if is_truthy "${MPTDC_ENABLE_POSTPLACE_SROUTE_CANDIDATE_PROBE:-0}"; then
...

{
  ...
  echo "create_ro_route_blockages: ${MPTDC_PNR_CREATE_RO_ROUTE_BLOCKAGES:-unset}"
  echo "ro_route_blockage_margin_um: ${MPTDC_RO_ROUTE_BLOCKAGE_MARGIN_UM:-unset}"
  echo "ro_route_blockage_layers: ${MPTDC_RO_ROUTE_BLOCKAGE_LAYERS:-unset}"
  echo "ro_route_blockage_open_sides: ${MPTDC_RO_ROUTE_BLOCKAGE_OPEN_SIDES:-unset}"
  ...
  echo "pg_strategy: ${MPTDC_PG_STRATEGY:-unset}"
  ...
  echo "stop_after_postplace_pre_route_sroute: ${MPTDC_STOP_AFTER_POSTPLACE_PRE_ROUTE_SROUTE:-unset}"
  ...
  echo "sroute_mode_profile: ${MPTDC_SROUTE_MODE_PROFILE:-unset}"
