...
apply_recovery_defaults() {
  ...
  if is_truthy "$MPTDC_PNR_FREE_INTERNAL_PLACEMENT"; then
    export MPTDC_REQUIRE_POSTPLACE_PRE_ROUTE_SROUTE_CLEAN="${MPTDC_REQUIRE_POSTPLACE_PRE_ROUTE_SROUTE_CLEAN:-0}"
    export MPTDC_ROUTE_GATE_SROUTE_RECOVERY="${MPTDC_ROUTE_GATE_SROUTE_RECOVERY:-1}"
    export MPTDC_POSTPLACE_PRE_ROUTE_ALLOW_DANGLING_ONLY="${MPTDC_POSTPLACE_PRE_ROUTE_ALLOW_DANGLING_ONLY:-1}"
  else
