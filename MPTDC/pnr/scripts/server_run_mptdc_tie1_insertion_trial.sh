...
EXPECTED_INSTANCE_PIN_SHA="${MPTDC_TIE1_TRIAL_EXPECTED_TARGET_SHA:-aede577d16cd1b844db07ade5acbe18a3ee886d2893f14dbab19499db3735b8e}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --source-failed-trial-run-id) SOURCE_FAILED_TRIAL_RUN_ID="${2:?missing --source-failed-trial-run-id value}"; shift 2 ;;
...
SOURCE_FAILED_TRIAL_DIR="$INNOVUS_WORK/$SOURCE_FAILED_TRIAL_RUN_ID"
TRACKED_FAILED_TRIAL_DIR="$REPO_ROOT/MPTDC/docs/server_snapshots/innovus/$SOURCE_FAILED_TRIAL_RUN_ID"
SOURCE_FAILED_TRIAL_GATE="$SOURCE_FAILED_TRIAL_DIR/reports/operator_gate_tie1_insertion_trial.rpt"
SOURCE_FAILED_TRIAL_STATUS="$SOURCE_FAILED_TRIAL_DIR/reports/tie1_insertion_trial_status.rpt"
SOURCE_FAILED_TRIAL_ACTION="$SOURCE_FAILED_TRIAL_DIR/reports/tie1_insertion_trial_action.rpt"
SOURCE_FAILED_TRIAL_TARGETS="$SOURCE_FAILED_TRIAL_DIR/manifests/tie1_instance_pin_targets.txt"
SOURCE_FAILED_TRIAL_INPUTS="$SOURCE_FAILED_TRIAL_DIR/manifests/tie1_insertion_trial_inputs.rpt"
SOURCE_FAILED_TRIAL_CHECKPLACE="$SOURCE_FAILED_TRIAL_DIR/reports/tie1_trial_baseline_check_place.rpt"
...
echo "SOURCE_FAILED_TRIAL_RUN_ID=$SOURCE_FAILED_TRIAL_RUN_ID"
echo "SOURCE_FAILED_TRIAL_DIR=$SOURCE_FAILED_TRIAL_DIR"
...
for rel in \
  ...
  live="$SOURCE_FAILED_TRIAL_DIR/$rel"
  tracked="$TRACKED_FAILED_TRIAL_DIR/$rel"
  if [[ ! -s "$live" || ! -s "$tracked" ]]; then
    echo "STOP: live or tracked Step 7I artifact missing: $rel"
    PREFLIGHT=FAIL
  elif ! cmp -s "$live" "$tracked"; then
    echo "STOP: live Step 7I artifact differs from published evidence: $rel"
    PREFLIGHT=FAIL
  fi
done
...
require_report_value "$SOURCE_FAILED_TRIAL_GATE" STEP TIE1_INSERTION_TRIAL
require_report_value "$SOURCE_FAILED_TRIAL_GATE" PROBE_RUN_ID "$PROBE_RUN_ID"
require_report_value "$SOURCE_FAILED_TRIAL_GATE" SOURCE_CHECKPOINT "$SOURCE_CHECKPOINT"
require_report_value "$SOURCE_FAILED_TRIAL_GATE" AUTHORIZATION EXACT_MPTDC_TIE1_HIGH_TRIAL
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INNOVUS_RC 1
require_report_value "$SOURCE_FAILED_TRIAL_GATE" TIE1_INSERTION_TRIAL_STATUS FAIL
require_report_value "$SOURCE_FAILED_TRIAL_GATE" ADD_TIE_SELECTION_MODE EXACT_INSTANCE_PIN_FILE
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_INPUT_GENERATION_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_COUNT 91
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_UNIQUE_COUNT 91
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_INVALID_COUNT 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_MATCH_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" SET_TIE_MODE_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" ADD_TIE_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" ADD_TIE_EFFECT_STATUS FAIL
require_report_value "$SOURCE_FAILED_TRIAL_GATE" ADD_TIE_EFFECT_REASON NO_ELIGIBLE_TARGET_WAS_CONNECTED
require_report_value "$SOURCE_FAILED_TRIAL_GATE" FINAL_CONNECTED_HIGH_TERM_COUNT 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" FINAL_DISCONNECTED_HIGH_TERM_COUNT 91
require_report_value "$SOURCE_FAILED_TRIAL_GATE" FINAL_TIE_NET_COUNT 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" FILLER_COUNT_AFTER 24797
require_report_value "$SOURCE_FAILED_TRIAL_GATE" BASELINE_DRC 1
require_report_value "$SOURCE_FAILED_TRIAL_GATE" BASELINE_SHORTS 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" BASELINE_REGULAR_CONNECTIVITY_BAD 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" BASELINE_SPECIAL_CONNECTIVITY_BAD 1
require_report_value "$SOURCE_FAILED_TRIAL_GATE" BASELINE_UNROUTED_NETS 0
require_report_value "$SOURCE_FAILED_TRIAL_GATE" SOURCE_CHECKPOINT_HASH_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" SAFE_INPUT_READ_ONLY_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_INPUT_READ_ONLY_STATUS PASS
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_SHA256_PRE "$EXPECTED_INSTANCE_PIN_SHA"
require_report_value "$SOURCE_FAILED_TRIAL_GATE" INSTANCE_PIN_TARGET_SHA256_POST "$EXPECTED_INSTANCE_PIN_SHA"
require_report_value "$SOURCE_FAILED_TRIAL_GATE" CANDIDATE_CHECKPOINT_STATUS FAIL
require_report_value "$SOURCE_FAILED_TRIAL_GATE" DECISION FAIL_STOP
require_report_value "$SOURCE_FAILED_TRIAL_GATE" NEXT_STAGE STOP_AND_REVIEW_PUBLISHED_EVIDENCE
checkplace_full_sites_passes "$SOURCE_FAILED_TRIAL_CHECKPLACE" "$EXPECTED_PLACEMENT_SITES" || {
  echo "STOP: Step 7I does not prove the exact full-site occupancy root cause"
  PREFLIGHT=FAIL
}
SOURCE_FAILED_TRIAL_TARGET_SHA="$(file_sha256 "$SOURCE_FAILED_TRIAL_TARGETS")"
[[ "$SOURCE_FAILED_TRIAL_TARGET_SHA" == "$EXPECTED_INSTANCE_PIN_SHA" ]] || {
  echo "STOP: Step 7I target file hash differs from the authorized target set"
  PREFLIGHT=FAIL
}
...
DELETE_FILLER_COMMAND_COUNT="$({ grep -Ec '^[[:space:]]*deleteFiller([[:space:]>]|$)' "$TRIAL_TCL" || true; } | tail -1)"
DELETE_FILLER_MASTER_OPTION_COUNT="$({ grep -Ec '^[[:space:]]*deleteFiller[[:space:]]+-cell[[:space:]]+\$filler_masters[[:space:]]+>[[:space:]]+\$delete_command_report[[:space:]]*$' "$TRIAL_TCL" || true; } | tail -1)"
...
INSTANCE_PIN_OPTION_COUNT="$({ grep -Ec '^[[:space:]]*-instancePin[[:space:]]+\$instance_pin_file([[:space:]\\]|$)' "$TRIAL_TCL" || true; } | tail -1)"
SET_FILLER_MODE_COMMAND_COUNT="$({ grep -Ec '^[[:space:]]*setFillerMode[[:space:]]+-add_fillers_with_drc[[:space:]]+false' "$TRIAL_TCL" || true; } | tail -1)"
ADD_FILLER_COMMAND_COUNT="$({ grep -Ec '^[[:space:]]*addFiller[[:space:]]+-cell[[:space:]]+\$filler_masters[[:space:]]+-prefix[[:space:]]+MPTDC_FILL' "$TRIAL_TCL" || true; } | tail -1)"
PG_REBIND_CALL_COUNT="$({ grep -Ec '^[[:space:]]*mptdc_signoff_apply_pg_connectivity([[:space:]{}]|$)' "$TRIAL_TCL" || true; } | tail -1)"
...
[[ "$INSTANCE_PIN_OPTION_COUNT" == 1 ]] || { echo "STOP: trial must bind addTieHiLo to the exact instance-pin file"; PREFLIGHT=FAIL; }
[[ "$SET_FILLER_MODE_COMMAND_COUNT" == 1 ]] || { echo "STOP: trial must set the DRC-safe filler mode exactly once"; PREFLIGHT=FAIL; }
[[ "$ADD_FILLER_COMMAND_COUNT" == 1 ]] || { echo "STOP: trial must invoke addFiller exactly once with the tracked masters"; PREFLIGHT=FAIL; }
[[ "$PG_REBIND_CALL_COUNT" == 1 ]] || { echo "STOP: trial must rebind PG connectivity exactly once"; PREFLIGHT=FAIL; }
...
echo "DELETE_FILLER_COMMAND_COUNT=$DELETE_FILLER_COMMAND_COUNT"
echo "DELETE_FILLER_MASTER_OPTION_COUNT=$DELETE_FILLER_MASTER_OPTION_COUNT"
echo "SET_MODE_COMMAND_COUNT=$SET_MODE_COMMAND_COUNT"
echo "ADD_TIE_COMMAND_COUNT=$ADD_TIE_COMMAND_COUNT"
echo "INSTANCE_PIN_OPTION_COUNT=$INSTANCE_PIN_OPTION_COUNT"
echo "SET_FILLER_MODE_COMMAND_COUNT=$SET_FILLER_MODE_COMMAND_COUNT"
echo "ADD_FILLER_COMMAND_COUNT=$ADD_FILLER_COMMAND_COUNT"
...
SOURCE_FAILED_TRIAL_EVIDENCE_SHA_PRE="$(tree_content_hash "$SOURCE_FAILED_TRIAL_DIR")"
TRACKED_FAILED_TRIAL_EVIDENCE_SHA_PRE="$(tree_content_hash "$TRACKED_FAILED_TRIAL_DIR")"
...
cp -p "$SOURCE_FAILED_TRIAL_GATE" "$MANIFEST_DIR/source_failed_trial_operator_gate.rpt"
cp -p "$SOURCE_FAILED_TRIAL_STATUS" "$MANIFEST_DIR/source_failed_trial_status.rpt"
cp -p "$SOURCE_FAILED_TRIAL_ACTION" "$MANIFEST_DIR/source_failed_trial_action.rpt"
cp -p "$SOURCE_FAILED_TRIAL_INPUTS" "$MANIFEST_DIR/source_failed_trial_inputs.rpt"
cp -p "$SOURCE_FAILED_TRIAL_CHECKPLACE" "$REPORT_DIR/source_failed_trial_baseline_check_place.rpt"
...
INSTANCE_PIN_TARGET_FILE="$MANIFEST_DIR/tie1_instance_pin_targets.txt"
awk -F '\t' '
  NR > 1 {
    target=$2
    if (target ~ /^\{.*\}$/) {
...
' "$MANIFEST_DIR/source_probe_flagged_term_inventory.tsv" \
  > "$INSTANCE_PIN_TARGET_FILE"
INSTANCE_PIN_TARGET_GENERATION_RC=$?
INSTANCE_PIN_TARGET_COUNT="$(awk 'NF {n++} END {print n+0}' "$INSTANCE_PIN_TARGET_FILE" 2>/dev/null)"
INSTANCE_PIN_TARGET_UNIQUE_COUNT="$(LC_ALL=C sort -u "$INSTANCE_PIN_TARGET_FILE" 2>/dev/null | awk 'NF {n++} END {print n+0}')"
INSTANCE_PIN_TARGET_INVALID_COUNT="$(awk '
  !NF || /[[:space:]]/ || $0 !~ /^.+\/[^/]+$/ {n++}
  END {print n+0}
' "$INSTANCE_PIN_TARGET_FILE" 2>/dev/null)"
INSTANCE_PIN_TARGET_SHA_PRE="$(file_sha256 "$INSTANCE_PIN_TARGET_FILE")"
INSTANCE_PIN_INPUT_GENERATION_STATUS=FAIL
if [[ "$INSTANCE_PIN_TARGET_GENERATION_RC" -eq 0 && \
      "$INSTANCE_PIN_TARGET_COUNT" == "$EXPECTED_HIGH_TERMS" && \
      "$INSTANCE_PIN_TARGET_UNIQUE_COUNT" == "$EXPECTED_HIGH_TERMS" && \
      "$INSTANCE_PIN_TARGET_INVALID_COUNT" == 0 && \
      "$INSTANCE_PIN_TARGET_SHA_PRE" == "$EXPECTED_INSTANCE_PIN_SHA" ]] && \
   cmp -s "$INSTANCE_PIN_TARGET_FILE" "$SOURCE_FAILED_TRIAL_TARGETS"; then
  INSTANCE_PIN_INPUT_GENERATION_STATUS=PASS
fi

echo "INSTANCE_PIN_INPUT_GENERATION_STATUS=$INSTANCE_PIN_INPUT_GENERATION_STATUS"
echo "INSTANCE_PIN_TARGET_COUNT=$INSTANCE_PIN_TARGET_COUNT"
echo "INSTANCE_PIN_TARGET_UNIQUE_COUNT=$INSTANCE_PIN_TARGET_UNIQUE_COUNT"
echo "INSTANCE_PIN_TARGET_INVALID_COUNT=$INSTANCE_PIN_TARGET_INVALID_COUNT"
echo "INSTANCE_PIN_TARGET_SHA256=$INSTANCE_PIN_TARGET_SHA_PRE"

{
  ...
  echo "SOURCE_FAILED_TRIAL_EVIDENCE_SHA256_PRE=$SOURCE_FAILED_TRIAL_EVIDENCE_SHA_PRE"
  echo "TRACKED_FAILED_TRIAL_EVIDENCE_SHA256_PRE=$TRACKED_FAILED_TRIAL_EVIDENCE_SHA_PRE"
  echo "SOURCE_FAILED_TRIAL_TARGET_SHA256=$SOURCE_FAILED_TRIAL_TARGET_SHA"
  ...
  echo "EXPECTED_INSTANCE_PIN_SHA256=$EXPECTED_INSTANCE_PIN_SHA"
  echo "RESTORE_COMMAND_COUNT=$RESTORE_COMMAND_COUNT"
  echo "DELETE_FILLER_COMMAND_COUNT=$DELETE_FILLER_COMMAND_COUNT"
  echo "DELETE_FILLER_MASTER_OPTION_COUNT=$DELETE_FILLER_MASTER_OPTION_COUNT"
  echo "SET_MODE_COMMAND_COUNT=$SET_MODE_COMMAND_COUNT"
  ...
  echo "CADENCE_ENV_RC=$CADENCE_ENV_RC"
  echo "CADENCE_ENV_STATUS=$CADENCE_ENV_STATUS"
  echo "ADD_TIE_SELECTION_MODE=EXACT_INSTANCE_PIN_FILE"
  echo "INSTANCE_PIN_TARGET_FILE=$INSTANCE_PIN_TARGET_FILE"
  echo "INSTANCE_PIN_TARGET_GENERATION_RC=$INSTANCE_PIN_TARGET_GENERATION_RC"
  echo "INSTANCE_PIN_INPUT_GENERATION_STATUS=$INSTANCE_PIN_INPUT_GENERATION_STATUS"
  echo "INSTANCE_PIN_TARGET_COUNT=$INSTANCE_PIN_TARGET_COUNT"
  echo "INSTANCE_PIN_TARGET_UNIQUE_COUNT=$INSTANCE_PIN_TARGET_UNIQUE_COUNT"
  echo "INSTANCE_PIN_TARGET_INVALID_COUNT=$INSTANCE_PIN_TARGET_INVALID_COUNT"
  echo "INSTANCE_PIN_TARGET_SHA256_PRE=$INSTANCE_PIN_TARGET_SHA_PRE"
...
export MPTDC_TIE1_TRIAL_INSTANCE_PIN_FILE="$INSTANCE_PIN_TARGET_FILE"
...
SOURCE_FAILED_TRIAL_EVIDENCE_SHA_POST="$(tree_content_hash "$SOURCE_FAILED_TRIAL_DIR")"
TRACKED_FAILED_TRIAL_EVIDENCE_SHA_POST="$(tree_content_hash "$TRACKED_FAILED_TRIAL_DIR")"
TRIAL_TCL_SHA_POST="$(file_sha256 "$TRIAL_TCL")"
INSTANCE_PIN_TARGET_SHA_POST="$(file_sha256 "$INSTANCE_PIN_TARGET_FILE")"
...
SOURCE_FAILED_TRIAL_EVIDENCE_READ_ONLY_STATUS=FAIL
TRIAL_TCL_READ_ONLY_STATUS=FAIL
CANDIDATE_CHECKPOINT_STATUS=FAIL
INSTANCE_PIN_INPUT_READ_ONLY_STATUS=FAIL
...
[[ "$SOURCE_FAILED_TRIAL_EVIDENCE_SHA_PRE" != MISSING && \
   "$SOURCE_FAILED_TRIAL_EVIDENCE_SHA_PRE" == "$SOURCE_FAILED_TRIAL_EVIDENCE_SHA_POST" && \
   "$TRACKED_FAILED_TRIAL_EVIDENCE_SHA_PRE" == "$TRACKED_FAILED_TRIAL_EVIDENCE_SHA_POST" ]] && SOURCE_FAILED_TRIAL_EVIDENCE_READ_ONLY_STATUS=PASS
[[ "$TRIAL_TCL_SHA_PRE" != MISSING && "$TRIAL_TCL_SHA_PRE" == "$TRIAL_TCL_SHA_POST" ]] && TRIAL_TCL_READ_ONLY_STATUS=PASS
[[ "$INSTANCE_PIN_TARGET_SHA_PRE" != MISSING && \
   "$INSTANCE_PIN_TARGET_SHA_PRE" == "$INSTANCE_PIN_TARGET_SHA_POST" ]] && INSTANCE_PIN_INPUT_READ_ONLY_STATUS=PASS
...
FILLER_RECYCLE_MODE="$(report_value "$TRIAL_REPORT" FILLER_RECYCLE_MODE)"
FILLER_DELETE_SELECTION_MODE="$(report_value "$TRIAL_REPORT" FILLER_DELETE_SELECTION_MODE)"
FILLER_DELETE_MASTER_COUNT="$(report_value "$TRIAL_REPORT" FILLER_DELETE_MASTER_COUNT)"
FILLER_DELETE_MASTER_SET="$(report_value "$TRIAL_REPORT" FILLER_DELETE_MASTER_SET)"
FILLER_DELETE_STATUS="$(report_value "$TRIAL_REPORT" FILLER_DELETE_STATUS)"
FILLER_DELETE_EFFECT_STATUS="$(report_value "$TRIAL_REPORT" FILLER_DELETE_EFFECT_STATUS)"
FILLER_COUNT_POST_DELETE="$(report_value "$TRIAL_REPORT" FILLER_COUNT_POST_DELETE)"
POST_DELETE_NONFILLER_STATUS="$(report_value "$TRIAL_REPORT" POST_DELETE_NONFILLER_FINGERPRINT_STATUS)"
POST_DELETE_ROUTE_STATUS="$(report_value "$TRIAL_REPORT" POST_DELETE_ROUTE_SIGNATURE_STATUS)"
ADD_TIE_SELECTION_MODE="$(report_value "$TRIAL_REPORT" ADD_TIE_SELECTION_MODE)"
INSTANCE_PIN_TARGET_FILE_STATUS="$(report_value "$TRIAL_REPORT" INSTANCE_PIN_TARGET_FILE_STATUS)"
INSTANCE_PIN_TARGET_COUNT_REPORTED="$(report_value "$TRIAL_REPORT" INSTANCE_PIN_TARGET_COUNT)"
INSTANCE_PIN_TARGET_UNIQUE_COUNT_REPORTED="$(report_value "$TRIAL_REPORT" INSTANCE_PIN_TARGET_UNIQUE_COUNT)"
INSTANCE_PIN_TARGET_INVALID_COUNT_REPORTED="$(report_value "$TRIAL_REPORT" INSTANCE_PIN_TARGET_INVALID_COUNT)"
INSTANCE_PIN_TARGET_MATCH_STATUS="$(report_value "$TRIAL_REPORT" INSTANCE_PIN_TARGET_MATCH_STATUS)"
TARGET_READBACK_MODE="$(report_value "$TRIAL_REPORT" TARGET_READBACK_MODE)"
TARGET_POINTER_SOURCE="$(report_value "$TRIAL_REPORT" TARGET_POINTER_SOURCE)"
SET_TIE_MODE_STATUS="$(report_value "$TRIAL_REPORT" SET_TIE_MODE_STATUS)"
ADD_TIE_STATUS="$(report_value "$TRIAL_REPORT" ADD_TIE_STATUS)"
ADD_TIE_EFFECT_STATUS="$(report_value "$TRIAL_REPORT" ADD_TIE_EFFECT_STATUS)"
ADD_TIE_EFFECT_REASON="$(report_value "$TRIAL_REPORT" ADD_TIE_EFFECT_REASON)"
POST_ADD_TARGET_POINTER_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_POINTER_COUNT)"
POST_ADD_TARGET_RESOLVED_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_RESOLVED_COUNT)"
POST_ADD_TARGET_MISSING_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_MISSING_COUNT)"
POST_ADD_TARGET_DUPLICATE_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_DUPLICATE_COUNT)"
POST_ADD_TARGET_UNEXPECTED_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_UNEXPECTED_COUNT)"
POST_ADD_TARGET_SET_STATUS="$(report_value "$TRIAL_REPORT" POST_ADD_TARGET_SET_STATUS)"
POST_ADD_CONNECTED_HIGH_TERM_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_CONNECTED_HIGH_TERM_COUNT)"
POST_ADD_DISCONNECTED_HIGH_TERM_COUNT="$(report_value "$TRIAL_REPORT" POST_ADD_DISCONNECTED_HIGH_TERM_COUNT)"
...
FINAL_TARGET_RESOLVED_COUNT="$(report_value "$TRIAL_REPORT" FINAL_TARGET_RESOLVED_COUNT)"
FINAL_TARGET_MISSING_COUNT="$(report_value "$TRIAL_REPORT" FINAL_TARGET_MISSING_COUNT)"
FINAL_TARGET_DUPLICATE_COUNT="$(report_value "$TRIAL_REPORT" FINAL_TARGET_DUPLICATE_COUNT)"
FINAL_TARGET_UNEXPECTED_COUNT="$(report_value "$TRIAL_REPORT" FINAL_TARGET_UNEXPECTED_COUNT)"
FINAL_TARGET_SET_STATUS="$(report_value "$TRIAL_REPORT" FINAL_TARGET_SET_STATUS)"
FINAL_FLAGGED_HIGH_TERM_COUNT="$(report_value "$TRIAL_REPORT" FINAL_FLAGGED_HIGH_TERM_COUNT)"
...
FINAL_FILLER_MASTER_SET_STATUS="$(report_value "$TRIAL_REPORT" FINAL_FILLER_MASTER_SET_STATUS)"
NONFILLER_FINGERPRINT_STATUS="$(report_value "$TRIAL_REPORT" NONFILLER_FINGERPRINT_STATUS)"
BASELINE_SITE_OCCUPANCY_STATUS="$(report_value "$TRIAL_REPORT" BASELINE_SITE_OCCUPANCY_STATUS)"
BASELINE_PLACEMENT_SITE_OCCUPIED="$(report_value "$TRIAL_REPORT" BASELINE_PLACEMENT_SITE_OCCUPIED)"
BASELINE_PLACEMENT_SITE_CAPACITY="$(report_value "$TRIAL_REPORT" BASELINE_PLACEMENT_SITE_CAPACITY)"
FINAL_SITE_OCCUPANCY_STATUS="$(report_value "$TRIAL_REPORT" FINAL_SITE_OCCUPANCY_STATUS)"
FINAL_PLACEMENT_SITE_OCCUPIED="$(report_value "$TRIAL_REPORT" FINAL_PLACEMENT_SITE_OCCUPIED)"
FINAL_PLACEMENT_SITE_CAPACITY="$(report_value "$TRIAL_REPORT" FINAL_PLACEMENT_SITE_CAPACITY)"
...
BASELINE_MARKER_RECONCILIATION_STATUS="$(report_value "$TRIAL_REPORT" BASELINE_DRC_MARKER_RECONCILIATION_STATUS)"
BASELINE_MARKER_RAW_GEOMETRY_COUNT="$(report_value "$TRIAL_REPORT" BASELINE_DRC_MARKER_RAW_GEOMETRY_COUNT)"
BASELINE_MARKER_LIVE_COUNT="$(report_value "$TRIAL_REPORT" BASELINE_DRC_MARKER_LIVE_COUNT)"
BASELINE_MARKER_STALE_COUNT="$(report_value "$TRIAL_REPORT" BASELINE_DRC_MARKER_STALE_COUNT)"
BASELINE_MARKER_UNMAPPED_COUNT="$(report_value "$TRIAL_REPORT" BASELINE_DRC_MARKER_UNMAPPED_COUNT)"
...
FINAL_MARKER_RECONCILIATION_STATUS="$(report_value "$TRIAL_REPORT" FINAL_DRC_MARKER_RECONCILIATION_STATUS)"
FINAL_MARKER_RAW_GEOMETRY_COUNT="$(report_value "$TRIAL_REPORT" FINAL_DRC_MARKER_RAW_GEOMETRY_COUNT)"
FINAL_MARKER_LIVE_COUNT="$(report_value "$TRIAL_REPORT" FINAL_DRC_MARKER_LIVE_COUNT)"
FINAL_MARKER_STALE_COUNT="$(report_value "$TRIAL_REPORT" FINAL_DRC_MARKER_STALE_COUNT)"
FINAL_MARKER_UNMAPPED_COUNT="$(report_value "$TRIAL_REPORT" FINAL_DRC_MARKER_UNMAPPED_COUNT)"
...
DRC_MARKER_RECONCILIATION_GATE_STATUS=FAIL
if [[ "$BASELINE_MARKER_RECONCILIATION_STATUS" == PASS && \
      "$FINAL_MARKER_RECONCILIATION_STATUS" == PASS && \
      "$BASELINE_MARKER_RAW_GEOMETRY_COUNT" =~ ^[0-9]+$ && \
      "$BASELINE_MARKER_LIVE_COUNT" =~ ^[0-9]+$ && \
      "$BASELINE_MARKER_STALE_COUNT" =~ ^[0-9]+$ && \
      "$BASELINE_MARKER_UNMAPPED_COUNT" =~ ^[0-9]+$ && \
      "$FINAL_MARKER_RAW_GEOMETRY_COUNT" =~ ^[0-9]+$ && \
      "$FINAL_MARKER_LIVE_COUNT" =~ ^[0-9]+$ && \
      "$FINAL_MARKER_STALE_COUNT" =~ ^[0-9]+$ && \
      "$FINAL_MARKER_UNMAPPED_COUNT" =~ ^[0-9]+$ && \
      "$BASELINE_MARKER_LIVE_COUNT" == 1 && \
      "$BASELINE_MARKER_STALE_COUNT" == 0 && \
      "$BASELINE_MARKER_UNMAPPED_COUNT" == 0 && \
      "$FINAL_MARKER_LIVE_COUNT" == 1 && \
      "$FINAL_MARKER_UNMAPPED_COUNT" == 0 ]] && \
   (( BASELINE_MARKER_RAW_GEOMETRY_COUNT ==
          BASELINE_MARKER_LIVE_COUNT + BASELINE_MARKER_STALE_COUNT &&
      FINAL_MARKER_RAW_GEOMETRY_COUNT ==
          FINAL_MARKER_LIVE_COUNT + FINAL_MARKER_STALE_COUNT )); then
...
if [[ "$INNOVUS_RC" -eq 0 && "$TRIAL_STATUS" == PASS && \
      ...
      "$FILLER_DELETE_MASTER_SET" == "$FILLER_MASTERS" && \
      "$FILLER_DELETE_STATUS" == PASS && "$FILLER_DELETE_EFFECT_STATUS" == PASS && \
      "$FILLER_COUNT_POST_DELETE" == 0 && \
      "$POST_DELETE_NONFILLER_STATUS" == PASS && "$POST_DELETE_ROUTE_STATUS" == PASS && \
      "$ADD_TIE_SELECTION_MODE" == EXACT_INSTANCE_PIN_FILE && \
      "$INSTANCE_PIN_INPUT_GENERATION_STATUS" == PASS && \
      "$INSTANCE_PIN_TARGET_FILE_STATUS" == PASS && \
      "$INSTANCE_PIN_TARGET_COUNT_REPORTED" == 91 && \
      "$INSTANCE_PIN_TARGET_UNIQUE_COUNT_REPORTED" == 91 && \
      "$INSTANCE_PIN_TARGET_INVALID_COUNT_REPORTED" == 0 && \
      ...
      "$INSTANCE_PIN_TARGET_SHA_PRE" == "$EXPECTED_INSTANCE_PIN_SHA" && \
      "$INSTANCE_PIN_TARGET_SHA_POST" == "$EXPECTED_INSTANCE_PIN_SHA" && \
      "$SET_TIE_MODE_STATUS" == PASS && "$ADD_TIE_STATUS" == PASS && \
      ...
      "$BASELINE_PLACEMENT_SITE_OCCUPIED" == "$EXPECTED_PLACEMENT_SITES" && \
      "$BASELINE_PLACEMENT_SITE_CAPACITY" == "$EXPECTED_PLACEMENT_SITES" && \
      "$FINAL_PLACEMENT_SITE_OCCUPIED" == "$EXPECTED_PLACEMENT_SITES" && \
      "$FINAL_PLACEMENT_SITE_CAPACITY" == "$EXPECTED_PLACEMENT_SITES" && \
...

{
  ...
  echo "FILLER_DELETE_SELECTION_MODE=$FILLER_DELETE_SELECTION_MODE"
  echo "FILLER_DELETE_MASTER_COUNT=$FILLER_DELETE_MASTER_COUNT"
  echo "FILLER_DELETE_MASTER_SET=$FILLER_DELETE_MASTER_SET"
  echo "FILLER_DELETE_STATUS=$FILLER_DELETE_STATUS"
  echo "FILLER_DELETE_EFFECT_STATUS=$FILLER_DELETE_EFFECT_STATUS"
  echo "FILLER_COUNT_POST_DELETE=$FILLER_COUNT_POST_DELETE"
  echo "POST_DELETE_NONFILLER_FINGERPRINT_STATUS=$POST_DELETE_NONFILLER_STATUS"
  echo "POST_DELETE_ROUTE_SIGNATURE_STATUS=$POST_DELETE_ROUTE_STATUS"
  echo "ADD_TIE_SELECTION_MODE=$ADD_TIE_SELECTION_MODE"
  echo "INSTANCE_PIN_INPUT_GENERATION_STATUS=$INSTANCE_PIN_INPUT_GENERATION_STATUS"
  echo "INSTANCE_PIN_TARGET_FILE=$INSTANCE_PIN_TARGET_FILE"
  echo "INSTANCE_PIN_TARGET_FILE_STATUS=$INSTANCE_PIN_TARGET_FILE_STATUS"
  echo "INSTANCE_PIN_TARGET_COUNT=$INSTANCE_PIN_TARGET_COUNT_REPORTED"
  echo "INSTANCE_PIN_TARGET_UNIQUE_COUNT=$INSTANCE_PIN_TARGET_UNIQUE_COUNT_REPORTED"
  echo "INSTANCE_PIN_TARGET_INVALID_COUNT=$INSTANCE_PIN_TARGET_INVALID_COUNT_REPORTED"
  echo "INSTANCE_PIN_TARGET_MATCH_STATUS=$INSTANCE_PIN_TARGET_MATCH_STATUS"
  echo "TARGET_READBACK_MODE=$TARGET_READBACK_MODE"
  echo "TARGET_POINTER_SOURCE=$TARGET_POINTER_SOURCE"
  echo "SET_TIE_MODE_STATUS=$SET_TIE_MODE_STATUS"
  echo "ADD_TIE_STATUS=$ADD_TIE_STATUS"
  echo "ADD_TIE_EFFECT_STATUS=$ADD_TIE_EFFECT_STATUS"
  echo "ADD_TIE_EFFECT_REASON=$ADD_TIE_EFFECT_REASON"
  echo "POST_ADD_TARGET_POINTER_COUNT=$POST_ADD_TARGET_POINTER_COUNT"
  echo "POST_ADD_TARGET_RESOLVED_COUNT=$POST_ADD_TARGET_RESOLVED_COUNT"
  echo "POST_ADD_TARGET_MISSING_COUNT=$POST_ADD_TARGET_MISSING_COUNT"
  echo "POST_ADD_TARGET_DUPLICATE_COUNT=$POST_ADD_TARGET_DUPLICATE_COUNT"
  echo "POST_ADD_TARGET_UNEXPECTED_COUNT=$POST_ADD_TARGET_UNEXPECTED_COUNT"
  echo "POST_ADD_TARGET_SET_STATUS=$POST_ADD_TARGET_SET_STATUS"
  echo "POST_ADD_CONNECTED_HIGH_TERM_COUNT=$POST_ADD_CONNECTED_HIGH_TERM_COUNT"
  ...
  echo "FINAL_TARGET_DUPLICATE_COUNT=$FINAL_TARGET_DUPLICATE_COUNT"
  echo "FINAL_TARGET_UNEXPECTED_COUNT=$FINAL_TARGET_UNEXPECTED_COUNT"
  echo "FINAL_TARGET_SET_STATUS=$FINAL_TARGET_SET_STATUS"
  echo "FINAL_FLAGGED_HIGH_TERM_COUNT=$FINAL_FLAGGED_HIGH_TERM_COUNT"
  ...
  echo "FINAL_FILLER_MASTER_SET_STATUS=$FINAL_FILLER_MASTER_SET_STATUS"
  echo "NONFILLER_FINGERPRINT_STATUS=$NONFILLER_FINGERPRINT_STATUS"
  echo "BASELINE_SITE_OCCUPANCY_STATUS=$BASELINE_SITE_OCCUPANCY_STATUS"
  echo "BASELINE_PLACEMENT_SITE_OCCUPIED=$BASELINE_PLACEMENT_SITE_OCCUPIED"
  echo "BASELINE_PLACEMENT_SITE_CAPACITY=$BASELINE_PLACEMENT_SITE_CAPACITY"
  echo "FINAL_SITE_OCCUPANCY_STATUS=$FINAL_SITE_OCCUPANCY_STATUS"
  echo "FINAL_PLACEMENT_SITE_OCCUPIED=$FINAL_PLACEMENT_SITE_OCCUPIED"
  echo "FINAL_PLACEMENT_SITE_CAPACITY=$FINAL_PLACEMENT_SITE_CAPACITY"
  ...
  echo "BASELINE_DRC_MARKER_RECONCILIATION_STATUS=$BASELINE_MARKER_RECONCILIATION_STATUS"
  echo "BASELINE_DRC_MARKER_RAW_GEOMETRY_COUNT=$BASELINE_MARKER_RAW_GEOMETRY_COUNT"
  echo "BASELINE_DRC_MARKER_LIVE_COUNT=$BASELINE_MARKER_LIVE_COUNT"
  echo "BASELINE_DRC_MARKER_STALE_COUNT=$BASELINE_MARKER_STALE_COUNT"
  echo "BASELINE_DRC_MARKER_UNMAPPED_COUNT=$BASELINE_MARKER_UNMAPPED_COUNT"
  ...
  echo "FINAL_DRC_MARKER_RECONCILIATION_STATUS=$FINAL_MARKER_RECONCILIATION_STATUS"
  echo "FINAL_DRC_MARKER_RAW_GEOMETRY_COUNT=$FINAL_MARKER_RAW_GEOMETRY_COUNT"
  echo "FINAL_DRC_MARKER_LIVE_COUNT=$FINAL_MARKER_LIVE_COUNT"
  echo "FINAL_DRC_MARKER_STALE_COUNT=$FINAL_MARKER_STALE_COUNT"
  echo "FINAL_DRC_MARKER_UNMAPPED_COUNT=$FINAL_MARKER_UNMAPPED_COUNT"
  echo "FINAL_DRC_MARKER_SIGNATURE_COUNT=$FINAL_MARKER_SIGNATURE_COUNT"
  echo "DRC_MARKER_RECONCILIATION_GATE_STATUS=$DRC_MARKER_RECONCILIATION_GATE_STATUS"
  ...
  echo "SOURCE_FAILED_TRIAL_EVIDENCE_READ_ONLY_STATUS=$SOURCE_FAILED_TRIAL_EVIDENCE_READ_ONLY_STATUS"
  echo "TRIAL_TCL_READ_ONLY_STATUS=$TRIAL_TCL_READ_ONLY_STATUS"
  echo "INSTANCE_PIN_INPUT_READ_ONLY_STATUS=$INSTANCE_PIN_INPUT_READ_ONLY_STATUS"
  echo "INSTANCE_PIN_TARGET_SHA256_PRE=$INSTANCE_PIN_TARGET_SHA_PRE"
  echo "INSTANCE_PIN_TARGET_SHA256_POST=$INSTANCE_PIN_TARGET_SHA_POST"
  echo "EXPECTED_INSTANCE_PIN_SHA256=$EXPECTED_INSTANCE_PIN_SHA"
  echo "SOURCE_FAILED_TRIAL_TARGET_SHA256=$SOURCE_FAILED_TRIAL_TARGET_SHA"
