...
proc mptdc_o10_cts {} {
    global o10
    mptdc_o10_msg "Running O10.1 CTS policy: clk_sys only, never RO phase clocks"
    ...
    puts $policy_fh "O10.1 CTS policy"
    puts $policy_fh "================="
    ...
    puts $guard_fh "O10.1 RO clock CTS guard"
    puts $guard_fh "========================"
    ...
    set o10(cts_status) "CTS_SKIPPED_FOR_FIRST_FEASIBILITY"
    set status_fh [open "$o10(reports_dir)/cts_status.rpt" w]
    puts $status_fh "O10.1 CTS status"
    puts $status_fh "================="
    ...
    if {[llength $clk_sys_objs] == 0} {
        puts $status_fh "CTS_STATUS=CTS_SKIPPED_FOR_FIRST_FEASIBILITY"
        ...
        if {$spec_created} {
            if {![catch {ccopt_design} err]} {
                set o10(cts_status) "CLK_SYS_ONLY_COMPLETE"
                puts $status_fh "CTS_STATUS=CLK_SYS_ONLY_COMPLETE"
            } else {
                puts $status_fh "CTS_STATUS=CTS_SKIPPED_FOR_FIRST_FEASIBILITY"
                puts $status_fh "ccopt_design_failed=$err"
                set fh [open "$o10(reports_dir)/CTS_SKIPPED.txt" w]
                puts $fh "CTS_SKIPPED_FOR_FIRST_FEASIBILITY"
            ...
            set fh [open "$o10(reports_dir)/CTS_SKIPPED.txt" w]
            puts $fh "CTS_SKIPPED_FOR_FIRST_FEASIBILITY"
            puts $fh "No clk_sys-only CTS command was accepted; generic CCOpt was not run."
            close $fh
        }
        close $status_fh
    }

    if {$o10(cts_status) eq "CLK_SYS_ONLY_COMPLETE"} {
        catch {optDesign -postCTS}
    }
    mptdc_o10_report_stage post_cts
    mptdc_o10_capture_candidates "$o10(reports_dir)/hold_post_cts.rpt" \
        "O10.1 hold post CTS" [list {report_timing -check_type hold -max_paths 100} {timeDesign -postCTS -hold}]
