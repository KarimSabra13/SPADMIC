...
set ::mptdc_test_manual_edit_type ""
set ::mptdc_test_manual_edit_layer ""
set ::mptdc_test_manual_edit_width 0.28
set ::mptdc_test_manual_edit_status routed
...
proc help {name args} {
    if {$name eq "setEditMode"} {
        if {[llength $args] != 2 || [lindex $args 0] ne ">"} {
            error "fixture requires redirected HELP output"
        }
        set fh [open [lindex $args 1] w]
        puts $fh {-type {regular|special|patch}}
        puts $fh {-layer <layer>}
        puts $fh {-width <value>}
        puts $fh {-status {cover|fixed|noshield|routed|shield|auto}}
        puts $fh {-stop_at_drc {true|false}}
        close $fh
        return
    }
...
proc dbGet {args} {
    if {$::mptdc_test_manual_mode} {
        ...
        if {[regexp {^net:(.+)\.pWires$} $expression -> net]} {
            set handles {}
            set idx 0
        ...
        if {[regexp {^mwire:([0-9]+)\.(.+)$} $expression -> idx attribute]} {
            ...
            switch -- $attribute {
                ...
                status {
                    if {[dict exists $row status]} {
                        return [dict get $row status]
                    }
                    return routed
                }
                shape {
                    if {[dict exists $row shape]} {
                        return [dict get $row shape]
                    }
                    return 0x0
                }
                length {
                    if {[dict exists $row length]} {
        ...
        if {[regexp {^mpwire:([0-9]+)\.(.+)$} $expression -> idx attribute]} {
            set row [lindex $::mptdc_test_manual_pwires $idx]
            switch -- $attribute {
                layer.name { return [dict get $row layer] }
                box { return [dict get $row box] }
                width { return [dict get $row width] }
...
proc setEditMode {args} {
    ...
    foreach spec {
        ...
        {-type mptdc_test_manual_edit_type}
        {-layer mptdc_test_manual_edit_layer}
...
proc mptdc_test_manual_rename_via {net point new_name} {
    set updated {}
    set renamed 0
    foreach row $::mptdc_test_manual_vias {
        if {[dict get $row net] eq $net &&
            [mptdc_ckpt_manual_point_equal [dict get $row point] $point]} {
            dict set row name $new_name
            incr renamed
        }
        lappend updated $row
    }
    if {$renamed != 1} {
        error "fixture expected one via to rename for $net at $point, found $renamed"
    }
...
proc editCommitRoute {x y} {
    ...
    set length 0.0
    set previous [lindex $::mptdc_test_manual_route_points 0]
    foreach point [lrange $::mptdc_test_manual_route_points 1 end] {
        set length [expr {$length +
            abs([lindex $point 0] - [lindex $previous 0]) +
            abs([lindex $point 1] - [lindex $previous 1])}]
        set previous $point
    }
    if {$::mptdc_test_manual_edit_type eq "patch"} {
        lappend ::mptdc_test_manual_pwires [dict create \
        ...
        set begin_ext 0.0
        set end_ext 0.0
        if {[llength $wire_points] == 2} {
            lassign [lindex $wire_points 0] x0 y0
            lassign [lindex $wire_points 1] x1 y1
            if {[mptdc_ckpt_manual_close $y0 $y1]} {
                set wire_box [list \
                    [expr {min($x0, $x1)}] [expr {$y0 - $half}] \
                    [expr {max($x0, $x1)}] [expr {$y0 + $half}]]
            } elseif {[mptdc_ckpt_manual_close $x0 $x1]} {
                set wire_box [list \
                    [expr {$x0 - $half}] [expr {min($y0, $y1)}] \
                    [expr {$x0 + $half}] [expr {max($y0, $y1)}]]
            }
        }
...
proc ecoRoute {args} {
    lappend ::mptdc_test_manual_command_calls [linsert $args 0 ecoRoute]
    set ::mptdc_test_local_ecoroute_called 1
    return {}
}
...
if {![catch {mptdc_ckpt_manual_two_minarea_landing_patch_v4} err] ||
    ![string match "*retired*" $err]} {
    error "minimum-area V4 retirement guard did not fail as expected: $err"
}
if {![catch {mptdc_ckpt_manual_two_minarea_landing_patch_v5} err] ||
    ![string match "*retired*" $err]} {
...
foreach tie_mode {tie1_trial tie1_replay} {
    ...
    set ::mptdc_test_manual_verify_count 0
    set ::mptdc_test_manual_snapshot_tuples [dict create \
        minarea_v8_${tie_mode}_pre [list 1 0 0 $minarea_01777_marker] \
        minarea_v8_${tie_mode}_post {0 0 0}]
    if {$tie_mode eq "tie1_trial"} {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_trial_v8]
    } else {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_replay_v8]
    }
    if {[dict get $tie_result status] ne "PASS" ||
        [dict get $tie_result mode] ne $tie_mode ||
        ![file exists [dict get $tie_result report]]} {
        error "minimum-area V8 $tie_mode entry point did not pass: $tie_result"
    }
    if {$::mptdc_test_manual_verify_count != 2 ||
        $::mptdc_test_set_db_calls ne {{mwire:0 .end_extension 0.255}}} {
        error "minimum-area V8 $tie_mode changed an unexpected object"
    }
...
foreach tie_mode {tie1_trial tie1_replay} {
    ...
    set ::mptdc_test_manual_verify_count 0
    set ::mptdc_test_manual_snapshot_tuples [dict create \
        minarea_v10_${tie_mode}_pre \
            [list 1 0 0 $minarea_01777_with_stale_marker] \
        minarea_v10_${tie_mode}_post {0 0 0}]
    if {$tie_mode eq "tie1_trial"} {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_trial_v10]
    } else {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_replay_v10]
    }
    if {[dict get $tie_result status] ne "PASS" ||
        [dict get $tie_result mode] ne $tie_mode ||
        ![file exists [dict get $tie_result report]]} {
        error "minimum-area V10 $tie_mode entry point did not pass: $tie_result"
    }
    if {$::mptdc_test_manual_verify_count != 2 ||
        [llength $::mptdc_test_manual_wires] != 2 ||
        [llength $::mptdc_test_set_db_calls] != 0} {
        error "minimum-area V10 $tie_mode changed an unexpected object"
    }
    ...
    if {[dict get $new_wire net] ne "u_core_n_57556" ||
        ...
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 0] {385.175 328.405}] ||
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 1] {385.035 328.405}]} {
        error "minimum-area V10 $tie_mode created the wrong tail: $new_wire"
    }
    set fh [open [dict get $tie_result report] r]
    set tie_text [read $fh]
    close $fh
    foreach expected [list \
        ...
        {N57556_FREE_END_TAIL_CANONICAL_STUB_STATUS=PASS} \
        {N57556_FREE_END_TAIL_PREEXISTING_WIRE_STATUS=PRESERVED} \
        {N57556_FREE_END_TAIL_POINTS_MATCH_STATUS=PASS} \
        {N57556_FREE_END_TAIL_LOCAL_BOX_STATUS=PASS} \
        {N57556_FREE_END_TAIL_STATUS=PASS} \
        ...
        {MANUAL_ECO_STATUS=PASS}] {
        if {[string first $expected $tie_text] < 0} {
            error "minimum-area V10 $tie_mode report is missing $expected"
        }
    }
    set route_start_count 0
    set route_commit_count 0
    set bounded_mode_count 0
    foreach call $::mptdc_test_manual_command_calls {
        if {[lindex $call 0] eq "editAddRoute"} {
            incr route_start_count
        }
        ...
        if {[lindex $call 0] eq "setEditMode" &&
            [lsearch -exact $call -type] >= 0 &&
            [lindex $call [expr {[lsearch -exact $call -type] + 1}]] eq "regular" &&
            [lsearch -exact $call -status] >= 0 &&
            [lindex $call [expr {[lsearch -exact $call -status] + 1}]] eq "fixed" &&
            [lsearch -exact $call -snap] >= 0 &&
            [lindex $call [expr {[lsearch -exact $call -snap] + 1}]] eq "false"} {
            incr bounded_mode_count
        }
        if {[lindex $call 0] in {editDelete editAddVia ecoRoute routeDesign globalDetailRoute detailRoute createRouteBlk editPowerVia}} {
            error "minimum-area V10 $tie_mode invoked a prohibited command: $call"
        }
    }
    if {$route_start_count != 1 || $route_commit_count != 1 ||
        $bounded_mode_count != 1} {
        error "minimum-area V10 $tie_mode did not issue one bounded regular tail edit"
    }
}
...
foreach tie_mode {tie1_trial tie1_replay} {
    ...
    set ::mptdc_test_manual_verify_count 0
    set ::mptdc_test_manual_snapshot_tuples [dict create \
        minarea_v11_${tie_mode}_pre \
            [list 1 0 0 $minarea_01777_with_stale_marker] \
        minarea_v11_${tie_mode}_post {0 0 0}]
    if {$tie_mode eq "tie1_trial"} {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_trial_v11]
    } else {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_replay_v11]
    }
    if {[dict get $tie_result status] ne "PASS" ||
        [dict get $tie_result mode] ne $tie_mode ||
        ![file exists [dict get $tie_result report]]} {
        error "minimum-area V11 $tie_mode entry point did not pass: $tie_result"
    }
    if {$::mptdc_test_manual_verify_count != 2 ||
        [llength $::mptdc_test_manual_wires] != 2 ||
        [llength $::mptdc_test_set_db_calls] != 0} {
        error "minimum-area V11 $tie_mode changed an unexpected object"
    }
    set new_wire [lindex $::mptdc_test_manual_wires 1]
    if {[dict get $new_wire net] ne "u_core_n_57556" ||
        [dict get $new_wire layer] ne "MET1" ||
        ![mptdc_ckpt_manual_close [dict get $new_wire width] 0.23] ||
        ![mptdc_ckpt_manual_close [dict get $new_wire length] 0.25] ||
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 0] {385.175 328.405}] ||
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 1] {385.175 328.155}]} {
        error "minimum-area V11 $tie_mode created the wrong tail: $new_wire"
    }
    set fh [open [dict get $tie_result report] r]
    set tie_text [read $fh]
    close $fh
    foreach expected [list \
        ...
        {REPAIR_REVISION=V11} \
        "V11_STAGE_MODE=$tie_mode" \
        {EXPECTED_STALE_GEOMETRY_MARKER_COUNT=1} \
        {PRE_MINAREA_MARKER_RECONCILIATION_STATUS=PASS} \
        {FREE_END_TAIL_LENGTH_UM=0.25} \
        {FREE_END_TAIL_DIRECTION=VERTICAL_DOWN} \
        {FREE_END_TAIL_START=385.175 328.405} \
        {FREE_END_TAIL_FINISH=385.175 328.155} \
        {V10_HORIZONTAL_TAIL_DISPOSITION=REJECTED_MINAREA_AND_FE_RC_5_0_SPACING} \
        {PREDICTED_CONNECTED_AREA_UM2=0.20875} \
        {ATTRIBUTE_EDIT_POLICY=NO_DB_ATTRIBUTE_EDITS} \
        {GEOMETRY_EDIT_POLICY=ONE_EXACT_TARGET_NET_REGULAR_FIXED_MET1_PERPENDICULAR_TAIL} \
        {N57556_FREE_END_TAIL_CANONICAL_STUB_STATUS=PASS} \
        {N57556_FREE_END_TAIL_DIRECTION=VERTICAL_DOWN} \
        {N57556_FREE_END_TAIL_PREEXISTING_WIRE_STATUS=PRESERVED} \
        {N57556_FREE_END_TAIL_POINTS_MATCH_STATUS=PASS} \
        {N57556_FREE_END_TAIL_LOCAL_BOX_STATUS=PASS} \
        {N57556_FREE_END_TAIL_STATUS=PASS} \
        ...
        {MANUAL_ECO_STATUS=PASS}] {
        if {[string first $expected $tie_text] < 0} {
            error "minimum-area V11 $tie_mode report is missing $expected"
        }
    }
    set route_start_count 0
    set route_commit_count 0
    ...
    foreach call $::mptdc_test_manual_command_calls {
        ...
        if {[lindex $call 0] in {editDelete editAddVia ecoRoute routeDesign globalDetailRoute detailRoute createRouteBlk editPowerVia}} {
            error "minimum-area V11 $tie_mode invoked a prohibited command: $call"
        }
    }
    if {$route_start_count != 1 || $route_commit_count != 1 ||
        $bounded_mode_count != 1} {
        error "minimum-area V11 $tie_mode did not issue one bounded perpendicular tail edit"
    }
}
...
foreach tie_mode {tie1_trial tie1_replay} {
    ...
    set ::mptdc_test_manual_verify_count 0
    set ::mptdc_test_manual_snapshot_tuples [dict create \
        minarea_v12_${tie_mode}_pre \
            [list 1 0 0 $minarea_01777_with_stale_marker] \
        minarea_v12_${tie_mode}_post {0 0 0}]
    if {$tie_mode eq "tie1_trial"} {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_trial_v12]
    } else {
        set tie_result [mptdc_ckpt_tie1_minarea_endext_replay_v12]
    }
    if {[dict get $tie_result status] ne "PASS" ||
        [dict get $tie_result mode] ne $tie_mode ||
        ![file exists [dict get $tie_result report]]} {
        error "minimum-area V12 $tie_mode entry point did not pass: $tie_result"
    }
    if {$::mptdc_test_manual_verify_count != 2 ||
        [llength $::mptdc_test_manual_wires] != 2 ||
        [llength $::mptdc_test_set_db_calls] != 0} {
        error "minimum-area V12 $tie_mode changed an unexpected object"
    }
    set new_wire [lindex $::mptdc_test_manual_wires 1]
    if {[dict get $new_wire net] ne "u_core_n_57556" ||
        [dict get $new_wire layer] ne "MET1" ||
        ![mptdc_ckpt_manual_close [dict get $new_wire width] 0.23] ||
        ![mptdc_ckpt_manual_close [dict get $new_wire length] 0.385] ||
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 0] {385.175 328.280}] ||
        ![mptdc_ckpt_manual_point_equal \
            [lindex [dict get $new_wire points] 1] {385.560 328.280}] ||
        ![mptdc_ckpt_manual_box_equal [dict get $new_wire box] \
            {385.175 328.165 385.560 328.395}]} {
        error "minimum-area V12 $tie_mode created the wrong shelf: $new_wire"
    }
    set fh [open [dict get $tie_result report] r]
    set tie_text [read $fh]
    close $fh
    foreach expected [list \
        ...
        {REPAIR_REVISION=V12} \
        "V12_STAGE_MODE=$tie_mode" \
        {VIA_OVERLAP_SHELF_LENGTH_UM=0.385} \
        {VIA_OVERLAP_SHELF_WIDTH_UM=0.23} \
        {VIA_OVERLAP_SHELF_DIRECTION=HORIZONTAL_RIGHT} \
        {VIA_OVERLAP_SHELF_START=385.175 328.280} \
        {VIA_OVERLAP_SHELF_FINISH=385.560 328.280} \
        {PREDICTED_NEW_WIRE_BOX=385.175 328.165 385.560 328.395} \
        {V11_PERPENDICULAR_TAIL_DISPOSITION=REJECTED_VIA1_O_NOTCH_SPACING} \
        {V11_OBSERVED_NOTCH_SPACING_UM=0.080} \
        {PREDICTED_CONNECTED_AREA_UM2=0.221075} \
        {GEOMETRY_EDIT_POLICY=ONE_EXACT_TARGET_NET_REGULAR_FIXED_MET1_VIA_OVERLAP_SHELF} \
        {N57556_VIA_OVERLAP_SHELF_CANONICAL_STUB_STATUS=PASS} \
        {N57556_VIA_OVERLAP_SHELF_EXPECTED_BOX=385.175 328.165 385.560 328.395} \
        {N57556_VIA_OVERLAP_SHELF_POINTS_MATCH_STATUS=PASS} \
        {N57556_VIA_OVERLAP_SHELF_BOX_MATCH_STATUS=PASS} \
        {N57556_VIA_OVERLAP_SHELF_LOCAL_BOX_STATUS=PASS} \
        ...
        {MANUAL_ECO_STATUS=PASS}] {
        if {[string first $expected $tie_text] < 0} {
            error "minimum-area V12 $tie_mode report is missing $expected"
        }
    }
    set route_start_count 0
    set route_commit_count 0
    foreach call $::mptdc_test_manual_command_calls {
        ...
        if {[lindex $call 0] in {editDelete editAddVia ecoRoute routeDesign globalDetailRoute detailRoute createRouteBlk editPowerVia}} {
            error "minimum-area V12 $tie_mode invoked a prohibited command: $call"
        }
    }
    if {$route_start_count != 1 || $route_commit_count != 1} {
