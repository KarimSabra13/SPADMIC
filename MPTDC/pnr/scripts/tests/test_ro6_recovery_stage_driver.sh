...
if grep -qx 'mptdc_ckpt_manual_two_minarea_landing_patch_v2' "$commands_file"; then
  ...
  cat > "$run/reports/min_area_landing_patch_v2.rpt" <<RPT
...
grep -qx 'REPAIR_METHOD=STAGED_BASE_STUBS_THEN_NORMALIZED_VIA_SIDE_EXTENSION_V2' "$MINAREA_REPAIR_REPORT"
...
grep -Fqx 'VIA_SIDE_POLICY=u_core_n_57556:ACTUAL_NORMALIZED_VIA_ENDPOINT->EAST_0.56;width=0.28' "$MINAREA_REPAIR_REPORT"
...
grep -qx 'BASE_CANONICAL_VIA_SIDE_STATUS=PASS' "$MINAREA_REPAIR_REPORT"
