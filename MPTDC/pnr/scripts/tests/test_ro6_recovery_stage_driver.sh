...
if grep -qx 'mptdc_ckpt_manual_two_minarea_landing_patch_v3' "$commands_file"; then
  ...
  cat > "$run/reports/min_area_landing_patch_v3.rpt" <<RPT
...
grep -qx 'REPAIR_METHOD=STAGED_BASE_STUBS_THEN_EXACT_MARKER_DERIVED_VIA_SIDE_EXTENSION_V3' "$MINAREA_REPAIR_REPORT"
...
grep -Fqx 'VIA_SIDE_POLICY=u_core_n_57556:EXACT_0.1777_MARKER_CENTERLINE_AT_X_385.56->EAST_0.56;width=0.28' "$MINAREA_REPAIR_REPORT"
grep -qx 'VIA_EDIT_POLICY=NO_EXPLICIT_VIA_COMMANDS_TOOL_CANONICALIZATION_ALLOWED' "$MINAREA_REPAIR_REPORT"
grep -qx 'VIA_OBJECT_POLICY=POST_BASE_VIA_HANDLE_NOT_REQUIRED_CONNECTIVITY_AND_MET1_GEOMETRY_ARE_AUTHORITATIVE' "$MINAREA_REPAIR_REPORT"
...
grep -qx 'BASE_VIA_SIDE_RESOLUTION_STATUS=PASS' "$MINAREA_REPAIR_REPORT"
