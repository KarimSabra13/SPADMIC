...
if grep -qx 'mptdc_ckpt_manual_two_minarea_landing_patch_v6r' "$commands_file"; then
  ...
  if [[ "${FAKE_MINAREA_V6R_CANONICALIZES:-0}" == 1 ]]; then
    ...
    target_special_status=FAIL
    target_regular_status=FAIL
    reserved_fill_status=FAIL
    post_minarea_marker_count=1
    final_drc=1
    final_status=REVIEW_REQUIRED
  fi
  cat > "$run/reports/min_area_landing_patch_v6r.rpt" <<RPT
...
CALIBRATED_STUB_STATUS=$calibrated_stub_status
...
CALIBRATED_STUB_POST_COVERAGE_STATUS=$calibrated_stub_status
TARGET_SPECIAL_OBJECT_STATUS=$target_special_status
...
grep -qx 'N57556_LANDING_REPRESENTATION_STATUS=UNCHANGED' "$ENDEXT_TRIAL_REPORT"
grep -qx 'TARGET_VIA_FINGERPRINT_STATUS=UNCHANGED' "$ENDEXT_TRIAL_REPORT"
...
grep -qx 'N57556_LANDING_REPRESENTATION_STATUS=UNCHANGED' "$MINAREA_REPAIR_REPORT"
grep -qx 'TARGET_VIA_FINGERPRINT_STATUS=UNCHANGED' "$MINAREA_REPAIR_REPORT"
