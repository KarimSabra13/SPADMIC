...
if grep -qx 'mptdc_ckpt_manual_two_minarea_landing_patch_v4' "$commands_file"; then
  ...
  cat > "$run/reports/min_area_landing_patch_v4.rpt" <<RPT
...
grep -qx 'REPAIR_METHOD=PROVEN_N57960_STUB_THEN_DIRECT_N57556_VIA1_OUTWARD_EXTENSION_V4' "$MINAREA_REPAIR_REPORT"
grep -Fqx 'BASE_STUB_POLICY=u_core_n_57960:MET1:363.72,358.12->364.56,358.12;width=0.28' "$MINAREA_REPAIR_REPORT"
grep -Fqx 'OUTWARD_PATCH_POLICY=u_core_n_57556:VIA1_o@385.56,328.44->386.12,328.44;width=0.28' "$MINAREA_REPAIR_REPORT"
grep -qx 'ANCHOR_POLICY=USE_ORIGINAL_ROUTED_VIA1_CENTER_NOT_DRC_RECTANGLE_CENTER' "$MINAREA_REPAIR_REPORT"
grep -qx 'VIA_EDIT_POLICY=NO_EXPLICIT_VIA_COMMANDS' "$MINAREA_REPAIR_REPORT"
...
grep -qx 'BASE_VIA_ANCHOR_STATUS=PASS' "$MINAREA_REPAIR_REPORT"
grep -qx 'POST_OUTWARD_PATCH_COVERED=1' "$MINAREA_REPAIR_REPORT"
