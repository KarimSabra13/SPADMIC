...
if grep -q 'minarea_endext' "$commands"; then
  initial_drc=1
  if grep -q '_trial_v12' "$commands"; then
    manual="$run_dir/reports/tie1_min_area_fixed_wire_endext_trial_v12.rpt"
  else
    manual="$run_dir/reports/tie1_min_area_fixed_wire_endext_replay_v12.rpt"
  fi
  cat > "$manual" <<'RPT'
...
V11_OBSERVED_NEW_WIRE_BOX=385.06 328.155 385.29 328.405
...
grep -qx 'REPAIR_REVISION=V12' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'MANUAL_ECO_MODE=CANONICAL_FIXED_MET1_VIA_OVERLAP_SHELF_V12' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'VIA_OVERLAP_SHELF_DIRECTION=HORIZONTAL_RIGHT' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'VIA_OVERLAP_SHELF_START=385.175 328.280' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'VIA_OVERLAP_SHELF_FINISH=385.560 328.280' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'PREDICTED_NEW_WIRE_BOX=385.175 328.165 385.560 328.395' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'N57556_VIA_OVERLAP_SHELF_BOX_MATCH_STATUS=PASS' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'PREDICTED_CONNECTED_AREA_UM2=0.221075' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
...
grep -qx 'TARGET_WIRE_HANDLE_STATUS=ONE_EXACT_ADDITION' \
  "$WORK/$TRIAL_RUN/reports/operator_gate_tie1_minarea_endext_trial.rpt"
grep -qx 'mptdc_ckpt_tie1_minarea_endext_trial_v12' \
