...
usage() {
  cat <<'USAGE'
Usage:
  server_run_mptdc_ro6_recovery_stage.sh --stage <pg-proof|pg-sweep|physical-pnr|route-minarea-probe|route-minarea-patch-trial|route-minarea-repair|route-geometry-probe|route-geometry-repair> [options]
...
Options:
  ...
  --source-pnr-run-id <id>
                         ...
                         route-minarea-patch-trial, route-minarea-repair,
...
case "$STAGE" in
  pg-proof|pg-sweep|physical-pnr|route-minarea-probe|route-minarea-patch-trial|route-minarea-repair|route-geometry-probe|route-geometry-repair) ;;
...
case "$STAGE" in
  pg-sweep)
    ...
    ;;
  route-minarea-probe|route-minarea-patch-trial|route-minarea-repair|route-geometry-probe|route-geometry-repair)
...
if [[ "$STAGE" == "pg-proof" ]]; then
  ...
  if [[ "$STAGE" == "route-minarea-repair" ]]; then
    [[ -n "$SOURCE_PATCH_TRIAL_RUN_ID" ]] || {
      echo "STOP: --source-patch-trial-run-id is required for route-minarea-repair"
      PREFLIGHT=FAIL
    }
...
if [[ -n "$SOURCE_PNR_RUN_ID" && "$STAGE" != "route-minarea-probe" && \
  ...
  echo "STOP: --source-pnr-run-id is only valid for route-minarea-probe, route-minarea-patch-trial, route-minarea-repair, route-geometry-probe, or route-geometry-repair"
  PREFLIGHT=FAIL
fi
if [[ -n "$SOURCE_PATCH_TRIAL_RUN_ID" && "$STAGE" != "route-minarea-repair" ]]; then
  echo "STOP: --source-patch-trial-run-id is only valid for route-minarea-repair"
  PREFLIGHT=FAIL
fi
...
if [[ "$STAGE" == "route-minarea-repair" ]]; then
  run_route_minarea_patch_stage "$SOURCE_PNR_CHECKPOINT" replay
  exit $?
fi
