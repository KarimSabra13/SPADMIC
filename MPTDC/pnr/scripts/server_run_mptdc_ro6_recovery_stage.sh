...
usage() {
  cat <<'USAGE'
Usage:
  server_run_mptdc_ro6_recovery_stage.sh --stage <pg-proof|pg-sweep|physical-pnr|route-geometry-repair> [options]
...
Options:
  ...
  --source-pnr-run-id <id>
                         Required for route-geometry-repair; identifies the
                         published 3-DRC/1-short physical PnR source.
...
case "$STAGE" in
  pg-proof|pg-sweep|physical-pnr|route-geometry-repair) ;;
  *) echo "ERROR: --stage must be pg-proof, pg-sweep, physical-pnr, or route-geometry-repair" >&2; usage >&2; exit 2 ;;
esac
...
if [[ "$STAGE" == "pg-proof" ]]; then
  ...
  [[ -n "$SOURCE_PNR_RUN_ID" ]] || { echo "STOP: --source-pnr-run-id is required for route-geometry-repair"; PREFLIGHT=FAIL; }
...
if [[ -n "$SOURCE_PNR_RUN_ID" && "$STAGE" != "route-geometry-repair" ]]; then
  echo "STOP: --source-pnr-run-id is only valid for route-geometry-repair"
  PREFLIGHT=FAIL
fi
