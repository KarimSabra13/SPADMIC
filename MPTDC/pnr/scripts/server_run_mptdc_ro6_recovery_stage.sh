...
usage() {
  cat <<'USAGE'
Usage:
  server_run_mptdc_ro6_recovery_stage.sh --stage <pg-proof|pg-sweep|physical-pnr|route-minarea-probe|route-geometry-probe|route-geometry-repair> [options]
...
case "$STAGE" in
  pg-proof|pg-sweep|physical-pnr|route-minarea-probe|route-geometry-probe|route-geometry-repair) ;;
  *) echo "ERROR: --stage must be pg-proof, pg-sweep, physical-pnr, route-minarea-probe, route-geometry-probe, or route-geometry-repair" >&2; usage >&2; exit 2 ;;
esac
...
case "$STAGE" in
  pg-sweep)
    ...
    ;;
  route-minarea-probe|route-geometry-probe|route-geometry-repair)
...
if [[ -n "$SOURCE_PNR_RUN_ID" && "$STAGE" != "route-minarea-probe" && \
  ...
  echo "STOP: --source-pnr-run-id is only valid for route-minarea-probe, route-geometry-probe, or route-geometry-repair"
  PREFLIGHT=FAIL
fi
