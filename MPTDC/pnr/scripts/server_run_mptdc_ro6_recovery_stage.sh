...
usage() {
  cat <<'USAGE'
Usage:
  server_run_mptdc_ro6_recovery_stage.sh --stage <pg-proof|pg-sweep|physical-pnr|route-geometry-probe|route-geometry-repair> [options]
...
Options:
  ...
  --source-pnr-run-id <id>
                         Required for route-geometry-probe and
                         route-geometry-repair; identifies the published
                         3-DRC/1-short physical PnR source.
...
run_route_geometry_probe_stage() {
  ...
  local commands_file="$RUN_DIR/manifests/route_geometry_probe.commands.tcl"
  local commands_evidence="$RUN_DIR/manifests/route_geometry_probe_commands.rpt"
  local probe_log="$RUN_DIR/logs/route_geometry_probe_driver.log"
  local status_report="$RUN_DIR/reports/checkpoint_repair_status.rpt"
  local probe_report="$RUN_DIR/reports/route_geometry_target_probe.rpt"
...
mptdc_ckpt_probe_target_geometry {u_core_n_66687 u_core_n_67240 u_core_n_57563}
COMMANDS
  {
    echo "# Exact read-only command payload supplied to Innovus"
    echo "SOURCE_PNR_RUN_ID=$SOURCE_PNR_RUN_ID"
    echo "SOURCE_CHECKPOINT=$source_checkpoint"
    echo "PROBE_METHOD=READ_ONLY_TARGET_NET_PIN_WIRE_VIA_PG_GEOMETRY"
    cat "$commands_file"
  } > "$commands_evidence"
  ...
  if [[ "$probe_rc" -eq 0 && "$command_status" == PASS && \
        ...
        "$final_drc" == 3 && "$final_shorts" == 1 && \
        "$final_regular" == 0 && "$final_special" == 1 && \
        "$final_special_raw" == 1 && "$final_special_non_ro" == 0 && \
        "$final_dangling_count" == 12 && "$final_status" == REVIEW_REQUIRED && \
        "$final_checkpoint_exists" == 1 && "$probe_status" == PASS && \
        "$target_net_count" == 3 && "$target_with_instterms" == 3 && \
        "$target_with_pin_geometry" == 3 && \
        "$target_instterms" =~ ^[0-9]+$ && "$target_instterms" -gt 0 && \
        "$target_instterm_pin_geometry" =~ ^[0-9]+$ && \
        "$target_instterm_pin_geometry" -gt 0 && \
        "$target_wires" =~ ^[0-9]+$ && "$target_wires" -gt 0 && \
        "$target_vias" =~ ^[0-9]+$ && \
        "$nearby_pg" =~ ^[0-9]+$ && "$nearby_pg" -gt 0 && \
        "$help_status" == PASS && "$help_pass_count" =~ ^[0-9]+$ && \
        "$help_pass_count" -gt 0 && "$schema_status" == PASS && \
        "$schema_pass_count" =~ ^[0-9]+$ && "$schema_pass_count" -gt 0 && \
  ...
  {
    echo "STEP=ROUTE_GEOMETRY_PROBE"
    echo "SOURCE_PNR_RUN_ID=$SOURCE_PNR_RUN_ID"
    echo "SOURCE_CHECKPOINT=$source_checkpoint"
  ...
  } | tee "$RUN_DIR/reports/operator_gate_route_geometry_probe.rpt"

  MPTDC_SNAPSHOT_MAX_TEXT_BYTES=2097152 \
    bash "$PUBLISHER" innovus "$RUN_ID" "$RUN_DIR" ROUTE_GEOMETRY_PROBE
  publish_rc=$?
  next_head="$(git rev-parse HEAD 2>/dev/null)"

  echo "RECOVERY_STAGE=ROUTE_GEOMETRY_PROBE"
  ...
  if [[ "$decision" == PASS_CONTINUE && "$publish_rc" -eq 0 ]]; then
    echo "NEXT_STAGE=REVIEW_ROUTE_GEOMETRY_PROBE"
...
case "$STAGE" in
  pg-proof|pg-sweep|physical-pnr|route-geometry-probe|route-geometry-repair) ;;
  *) echo "ERROR: --stage must be pg-proof, pg-sweep, physical-pnr, route-geometry-probe, or route-geometry-repair" >&2; usage >&2; exit 2 ;;
esac
...
case "$STAGE" in
  pg-sweep)
    ...
    ;;
  route-geometry-probe|route-geometry-repair)
...
if [[ -n "$SOURCE_PNR_RUN_ID" && "$STAGE" != "route-geometry-probe" && \
      "$STAGE" != "route-geometry-repair" ]]; then
  echo "STOP: --source-pnr-run-id is only valid for route-geometry-probe or route-geometry-repair"
  PREFLIGHT=FAIL
fi
...
if [[ "$STAGE" == "route-geometry-probe" ]]; then
  run_route_geometry_probe_stage "$SOURCE_PNR_CHECKPOINT"
  exit $?
fi
