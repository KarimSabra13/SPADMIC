...
RUN_ID="${1:-20260604_o10_typical_feasibility}"
RESULT_DIR="$REPO_ROOT/results/innovus/$RUN_ID"
SNAPSHOT_TAG="innovus_o10_typical_feasibility_${RUN_ID}"
...
OVERLAY_SDC="${MPTDC_O10_SDC_OVERLAY:-$MPTDC_DIR/syn/inputs/mptdc_osc_typical_r750_delta5.sdc}"
...
mkdir -p "$LOG_DIR" "$RESULT_DIR/manifests"

{
  echo "# O10 Innovus Typical Feasibility Run"
  ...
  echo "labels: O10_INNOVUS_TYPICAL_FEASIBILITY NOT_MMMC_SIGNOFF NOT_FINAL_SIGNOFF NOT_TAPEOUT_READY"
  echo
...
require_file "O9 R750 overlay SDC" "$OVERLAY_SDC"
...
if [[ "$INPUT_RC" != "0" ]]; then
  ...
  (
    cd "$SCRIPT_DIR"
    innovus -nowin -init innovus_o10_init.tcl -log "$LOG_DIR/innovus_o10.log"
  ) 2>&1 | tee -a "$RUN_LOG"
  INNOVUS_RC=${PIPESTATUS[0]}
fi

echo "[SNAPSHOT] Collecting O10 snapshot into $SNAPSHOT_DIR" | tee -a "$RUN_LOG"
...
{
  echo "# O10 Innovus Typical Feasibility Summary"
  echo
  echo "- Run ID: \`$RUN_ID\`"
