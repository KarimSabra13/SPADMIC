...
grep -E '^(RO6_OA_VDD_EXPORT_PROBE_STATUS|SOURCE_MISMATCH_CLASSIFICATION|SOURCE_GDS_HASH_STATUS|SOURCE_CDL_HASH_STATUS|SOURCE_LINEAGE_STATUS|SOURCE_EXPORT_READ_ONLY_STATUS|STREAM_COLLATERAL_READ_ONLY_STATUS|XSTREAM_LOG_BINDING_STATUS|XSTREAM_LOG_COMPLETION_STATUS|XSTREAM_TRANSLATION_ZERO_ERROR_STATUS|XSTREAM_WRAPPER_COMPLETION_STATUS|OA_PROBE_CLASSIFICATION_STATUS|OA_READ_ONLY_STATUS|OA_VDD_EXPORT_DIAGNOSIS|OA_TERMINAL_CONTRACT_STATUS|OA_VDD_NET_ASSOCIATED_SHAPE_COUNT|OA_GOLDEN_VDD_PIN_OVERLAP_VDD_SHAPE_COUNT|OA_VDD_PIN_IGNORED_LPP_COUNT|OA_VDD_LABEL_IGNORED_LPP_COUNT|SIGNOFF_ELIGIBLE|DECISION|PUBLISH_RC|NEXT_EXPECTED_HEAD|NEXT_STAGE)=' \
  "$DRIVER_LOG" 2>/dev/null | tail -30
grep -E '^(OA_(SUMMARY_TERMINAL_COUNT|EXPECTED_TERMINAL_(SET|SUBSET)_STATUS|UNEXPECTED_TERMINAL_(COUNT|SET)|EMPTY_GLOBAL_ALIAS_STATUS|TERMINAL_CONTRACT_STATUS|VDD_(TERMINAL_COUNT|PIN_COUNT|PIN_FIG_COUNT|NET_SET|PIN_LPP_SET|PIN_BOX_SET|EXPLICIT_LABEL_COUNT|GLOBAL_LABEL_SHAPE_COUNT|PIN_FIG_TEXT_COUNT|LABEL_LPP_SET|LABEL_BOX_SET|LIKE_LABEL_COUNT|LIKE_LABEL_TEXT_SET|LIKE_LABEL_LPP_SET|NET_ASSOCIATED_SHAPE_COUNT)|VSS_(TERMINAL_COUNT|PIN_COUNT|PIN_FIG_COUNT|NET_SET|PIN_LPP_SET|PIN_BOX_SET|EXPLICIT_LABEL_COUNT|GLOBAL_LABEL_SHAPE_COUNT|PIN_FIG_TEXT_COUNT|LABEL_LPP_SET|LABEL_BOX_SET|LIKE_LABEL_COUNT|LIKE_LABEL_TEXT_SET)|GOLDEN_VDD_PIN_(BOX|OVERLAP_SHAPE_COUNT|OVERLAP_VDD_SHAPE_COUNT)|VDD_EXPORT_DIAGNOSIS|VDD_EXPORT_REVIEW_ACTION)|OA_PROBE_CLASSIFICATION_STATUS)=' \
  "$PROBE_DIR/reports/oa_ro6_vdd_export_classification.rpt" 2>/dev/null
cat "$PROBE_DIR/reports/oa_ro6_supply_nets.tsv" 2>/dev/null
cat "$PROBE_DIR/reports/oa_ro6_vdd_candidate_shapes.tsv" 2>/dev/null
echo "FINAL_HEAD=$(git rev-parse HEAD 2>/dev/null)"
```

A complete probe normally returns driver RC zero,
`RO6_OA_VDD_EXPORT_PROBE_STATUS=PASS`, both source hashes and lineage `PASS`,
XStream binding, translation-zero-error, and completion statuses `PASS`,
`XSTREAM_WRAPPER_COMPLETION_STATUS=PASS` or `MISSING_OPTIONAL`,
`OA_PROBE_CLASSIFICATION_STATUS=PASS`,
...
The first complete probe,
`20260827_mptdc_ro6_oa_vdd_export_probe_135213`, proved that source hashes,
lineage, XStream completion, and all read-only gates pass. It also isolated the
OA defect: all 19 required terminals exist, the only extra terminals are empty
`gnd!` and `vdd!` aliases, `VSS` has two pin figures and two exact labels, but
`VDD` has no pin figure and no label. The XStream map did not discard a VDD
object because no VDD pin or label existed to export. The revised probe records
current VDD-associated top-level geometry and overlap with the committed
...
For this IC23.1 checkpoint, the immutable DRC identity is the `Geometry`
subset of the marker table; retained `Antenna` and `Connectivity` markers are
outside the one-violation `verify_drc` total. When `reportRoute` omits an
explicit unrouted count, zero is accepted only from its two exact zero-route
