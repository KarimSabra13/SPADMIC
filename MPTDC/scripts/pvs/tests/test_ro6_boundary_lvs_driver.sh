...
NO_TIES_RC=$?
set -e
test "$NO_TIES_RC" -ne 0
grep -Fq 'source is not the attributable physical PG/tie LVS contract' \
  "$TMP_ROOT/no_ties.stdout"
grep -qx 'DECISION=FAIL_STOP' "$TMP_ROOT/no_ties.stdout"
