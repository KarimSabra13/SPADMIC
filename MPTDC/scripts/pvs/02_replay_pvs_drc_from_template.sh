...
NEW_DRC_RUN="${NEW_DRC_RUN:-$NEW_BASE/pvs_drc/mptdc_axis_core_merged_script}"
...
PATCH_FILES=("$NEW_DRC_RUN/run.pvs" "$NEW_DRC_RUN/.config.rul" "$NEW_DRC_RUN/.technology.rul")
...
} | tee "$NEW_BASE/manifests/pvs_drc_replay_manifest.txt"

{
  ...
  grep -RniE 'mptdc_axis_core_merged_stdcell_ro6|xh018_DRC|metalswitch|DRC|drc|rulesFile|GDS|gds' \
    "$NEW_DRC_RUN/run.pvs" "$NEW_DRC_RUN/.config.rul" "$NEW_DRC_RUN/.technology.rul" | sed -n '1,360p' || true
} | tee "$NEW_BASE/reports/pvs_drc_replay_preflight.rpt"

if [[ "$DRY_RUN" == "1" ]]; then
  echo "PVS_DRC_REPLAY_STATUS=DRY_RUN_READY" | tee "$NEW_BASE/reports/pvs_drc_status.rpt"
  exit 0
fi
...
PVS_RC=${PIPESTATUS[0]}
echo "PVS_DRC_RC=$PVS_RC" | tee "$NEW_BASE/reports/pvs_drc_status.rpt"
...
grep -RIna --binary-files=without-match \
  ...
  "$NEW_DRC_RUN" | head -2000 | tee "$NEW_BASE/reports/pvs_drc_result_scan.txt" || true

"$SCRIPT_DIR/04_correlate_innovus_pvs_drc.py" \
  --pvs-root "$NEW_DRC_RUN" \
  --out "$NEW_BASE/reports/pvs_drc_vs_innovus_mar4.rpt"
