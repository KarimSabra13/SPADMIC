...

{
  ...
  echo "  MPTDC_PD_HIT_TO_NFAST_MAX_DELAY_NS=${MPTDC_PD_HIT_TO_NFAST_MAX_DELAY_NS:-1.10}"
  echo "  MPTDC_PD_HIT_TO_NFAST_MAX_TRANSITION_NS=${MPTDC_PD_HIT_TO_NFAST_MAX_TRANSITION_NS:-0.25}"
...
mirror_final_report_artifacts() {
  ...
  for file in \
    ...
    pd_hit_to_nfast_local_path_pairs.csv; do
...
write_final_readiness() {
  local out="$RESULT_DIR/final_typical_genus_readiness.md"
  {
    ...
    emit_check_text "PD hit-to-nfast local max delay" OK "$PD_HIT_TO_NFAST_LOCAL_SET_MAX_DELAY_RESULT"
    emit_check_text "PD hit-to-nfast local max transition" OK "$PD_HIT_TO_NFAST_LOCAL_SET_MAX_TRANSITION_RESULT"
...
if [[ ( "$PD_HIT_TO_NFAST_LOCAL_REPAIR_ENABLE" == "1" || "$PD_HIT_TO_NFAST_LOCAL_REPAIR_ENABLE" == "true" ) \
    ...
    && "$PD_HIT_TO_NFAST_LOCAL_SET_MAX_DELAY_RESULT" == "OK" \
    && "$PD_HIT_TO_NFAST_LOCAL_SET_MAX_TRANSITION_RESULT" == "OK" \
