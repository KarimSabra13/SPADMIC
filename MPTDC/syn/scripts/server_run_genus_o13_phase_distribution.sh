...
if [[ "$MPTDC_GENUS_REPAIR6_LOCALTAG_PRESERVE_CLOSE" == "1" ]]; then
...
write_sdc_failure_report() {
  ...
  if [[ -f "$GENUS_LOG" ]]; then
    ...
    command_failure_count=$((sdc_235_count + tui_61_count + invalid_object_count + failed_command_count + fatal_count))
  fi
  {
...
write_final_readiness() {
  local out="$RESULT_DIR/final_typical_genus_readiness.md"
  {
    ...
    emit_check "SDC-235 count" 0 "$SDC_235_COUNT"
    emit_check "TUI-61 count" 0 "$TUI_61_COUNT"
