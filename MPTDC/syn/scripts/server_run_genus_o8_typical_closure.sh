...
RUN_ID="${1:-$(date +%Y%m%d_%H%M%S)_o8_typical_closure}"
...
O8_SDC="${O8_SDC_PATH:-$SYN_DIR/inputs/mptdc_osc_typical_from_screenshot.sdc}"
O8_BASE_FILELIST="${O8_FILELIST_PATH:-$SYN_DIR/filelist_o5_pd_stdcell_closure.f}"
O8_MODE="${O8_MODE:-typical_closure}"
O8_GENUS_EFFORT="closure"
...
case "$O8_MODE" in
  typical_closure|O8A_typical_closure)
    O8_MODE="typical_closure"
    O8_STATUS_LABEL="O8A_TYPICAL_FAST_CLOSURE"
    O8_NFAST_ENCODING="raw_lfsr_tag"
    O8_EXTRA_DEFINE=""
    ;;
  relax_fast_tag_preserve|O8B_typical_closure_relax_fast_tag_preserve)
    O8_MODE="relax_fast_tag_preserve"
    O8_STATUS_LABEL="O8B_TYPICAL_CLOSURE_RELAX_FAST_TAG_PRESERVE"
    O8_NFAST_ENCODING="raw_lfsr_tag"
    O8_EXTRA_DEFINE="+define+MPTDC_RELAX_FAST_TAG_PRESERVE"
    ;;
  raw_galois_tag|O8C_typical_fast_feasibility_raw_galois_tag)
    O8_MODE="raw_galois_tag"
    O8_STATUS_LABEL="O8C_TYPICAL_FAST_FEASIBILITY_RAW_GALOIS_TAG"
    O8_NFAST_ENCODING="raw_galois_tag"
    O8_EXTRA_DEFINE="+define+MPTDC_FAST_TAG_GALOIS"
    ;;
  *)
    echo "ERROR: unsupported O8_MODE=$O8_MODE" >&2
    echo "Allowed: typical_closure, relax_fast_tag_preserve, raw_galois_tag" >&2
    exit 2
    ;;
esac
...
if [[ ! -f "$O8_BASE_FILELIST" ]]; then
  echo "ERROR: missing O8 base filelist: $O8_BASE_FILELIST" | tee "$GENUS_LOG"
  exit 2
fi

O8_FILELIST="$O8_BASE_FILELIST"
if [[ -n "$O8_EXTRA_DEFINE" ]]; then
  O8_FILELIST="$RESULT_DIR/filelist_${RUN_ID}.f"
  {
    echo "// Generated by $(basename "$0") for $O8_STATUS_LABEL"
    echo "$O8_EXTRA_DEFINE"
    sed '/^+define+MPTDC_FAST_TAG_GALOIS$/d;/^+define+MPTDC_RELAX_FAST_TAG_PRESERVE$/d' "$O8_BASE_FILELIST"
  } > "$O8_FILELIST"
fi

{
  echo "# O8 Typical Fast Closure Genus Run"
  ...
  echo "mode: $O8_MODE"
  echo "flavor: $O8_STATUS_LABEL"
  echo "signoff_status: FEASIBILITY_ONLY_TYPICAL_NOT_FOR_SIGNOFF"
  echo "nfast_encoding: $O8_NFAST_ENCODING"
  echo
...
if [[ -f "$ENV_FILE" ]]; then
  ...
  echo "ERROR: missing O1/O8 env file: $ENV_FILE" | tee -a "$GENUS_LOG"
  INPUT_RC=2
fi
...
for required in "$REAL_LEF" "$REAL_LIB" "$O8_SDC" "$O8_FILELIST" "$MODEL_FILE"; do
  if [[ ! -f "$required" ]]; then
    echo "ERROR: missing required O8 input: $required" | tee -a "$GENUS_LOG"
    INPUT_RC=2
  fi
done
...
export MPTDC_READ_HDL_LIST="$O8_FILELIST"
export MPTDC_OSC_PD_USE_PROVISIONAL=0
export MPTDC_OSC_PD_USE_PROVISIONAL_LIBERTY=0
export MPTDC_OSC_PD_SDC_OVERLAY="$O8_SDC"
export O1_RUN_FLAVOR="$O8_STATUS_LABEL"
export O8_TIMING_MODE="$O8_MODE"
export GENUS_EFFORT="$O8_GENUS_EFFORT"
export MPTDC_OPT_GOAL="o8_${O8_MODE}_${O8_GENUS_EFFORT}"
export MPTDC_OSC_SLOW_PERIOD_NS="${O8_OSC_SLOW_PERIOD_NS:-1.000}"
export MPTDC_OSC_FAST_PERIOD_NS="${O8_OSC_FAST_PERIOD_NS:-0.900}"
export MPTDC_OSC_SLOW_TAP_STEP_NS="${O8_OSC_SLOW_TAP_STEP_NS:-0.055}"
export MPTDC_OSC_FAST_TAP_STEP_NS="${O8_OSC_FAST_TAP_STEP_NS:-0.050}"
export MPTDC_ENABLE_CLOCK_GATING="${O8_ENABLE_CLOCK_GATING:-0}"
export MPTDC_ALLOW_ICG_DONT_USE_OVERRIDE="${O8_ALLOW_ICG_DONT_USE_OVERRIDE:-0}"
export MPTDC_ALLOW_DISCRETE_CLOCK_GATING="${O8_ALLOW_DISCRETE_CLOCK_GATING:-0}"
export MPTDC_RELAX_PD_PRESERVE="${O8_RELAX_PD_PRESERVE:-1}"

{
  ...
  echo "  O8_SDC=$O8_SDC"
  echo "  O8_FILELIST=$O8_FILELIST"
  echo "  O8_BASE_FILELIST=$O8_BASE_FILELIST"
  echo "  O8_MODE=$O8_MODE"
  echo "  O8_EXTRA_DEFINE=${O8_EXTRA_DEFINE:-none}"
...
GENUS_RC="$INPUT_RC"
if [[ "$INPUT_RC" == "0" ]]; then
  echo "[GENUS_O8] Cleaning generated synthesis outputs/reports for a non-stale run" | tee -a "$GENUS_LOG"
  rm -rf "$SYN_DIR/reports/synthesis" "$SYN_DIR/outputs"
  mkdir -p "$SYN_DIR/reports" "$SYN_DIR/outputs" "$SYN_DIR/logs"

  echo "[GENUS_O8] Starting TC-only closure flow: mode=$O8_MODE effort=$GENUS_EFFORT" | tee -a "$GENUS_LOG"
  (
    cd "$SCRIPT_DIR"
    genus -files genus.tcl -log "../logs/genus_o8_${O8_MODE}_${GENUS_EFFORT}.log"
  ) 2>&1 | tee -a "$GENUS_LOG"
  GENUS_RC=${PIPESTATUS[0]}
fi

echo "[SNAPSHOT] Collecting Genus O8 snapshot into $SNAPSHOT_DIR" | tee -a "$GENUS_LOG"
SNAPSHOT_RC=0
...
if command -v python3 >/dev/null 2>&1; then
  ...
  python3 - "$RESULT_DIR" > "$RESULT_DIR/o8_class_wns_summary.txt" <<'PY' || true
import csv
import math
import sys
...

{
  ...
  echo "- Signoff status: \`FEASIBILITY_ONLY_TYPICAL_NOT_FOR_SIGNOFF\`"
  ...
  echo "- SDC overlay: \`$O8_SDC\`"
  echo "- HDL filelist: \`$O8_FILELIST\`"
  echo "- NFAST encoding: \`$O8_NFAST_ENCODING\`"
  echo "- Extra define: \`${O8_EXTRA_DEFINE:-none}\`"
  ...
  echo
  echo "O8_STATUS=$STATUS"
  echo "STATUS_LABEL=$O8_STATUS_LABEL"
  echo "FINAL_SIGNOFF=NO"
  echo
  if [[ -f "$RESULT_DIR/o8_class_wns_summary.txt" ]]; then
    echo "## WNS/TNS by Class and Family"
    cat "$RESULT_DIR/o8_class_wns_summary.txt"
    echo
  fi
  echo "## Key Files"
  for file in \
