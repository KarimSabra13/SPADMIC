...
class FastTagMetadata:
    ...
    def as_dict(self) -> dict[str, object]:
        return {
            ...
            "tag_decode_mode": self.tag_decode_mode,
...
def generate_lfsr_sequence(width: int = DEFAULT_WIDTH, seed: int = DEFAULT_SEED) -> list[int]:
    ...
    for _ in range(count):
        ...
        if state in seen:
            raise ValueError(f"LFSR repeated early at state {state}")
        seq.append(state)
        seen.add(state)
        state = lfsr_next(state, width)
    if state != seed:
        raise ValueError("LFSR did not return to seed after full sequence")
    return seq


def build_tag_to_index_table(width: int = DEFAULT_WIDTH, seed: int = DEFAULT_SEED) -> dict[int, int]:
    return {tag: idx for idx, tag in enumerate(generate_lfsr_sequence(width, seed))}
...
def decode_raw_tag(
    ...
    *,
    ...
    table = build_tag_to_index_table(width, seed)
    if raw_tag not in table:
        raise ValueError(f"Invalid raw LFSR tag 0x{raw_tag:x} for width={width}, seed={seed}")
...
def annotate_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        nf = int(out.get("nf", 0))
        out["nfast_raw_tag"] = raw if mode == RAW_LFSR_TAG else ""
...
def main() -> int:
    ...
    parser.add_argument("--mode", choices=[RAW_LFSR_TAG, LEGACY_BINARY_NFAST], default=RAW_LFSR_TAG)
