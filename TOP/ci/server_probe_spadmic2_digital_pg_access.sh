...
if [ "$DIAGNOSTIC_CREATE_RC" = "0" ]; then
    {
        ...
        echo "NEXT_GATE=RETURN_CORRECTED_FLOORPLAN_AND_PG_PAIR_FOR_REVIEW"
    } > "$EVIDENCE_STATUS"
fi
...
for REPORT in \
    ...
    "$PROCESSED_ROOT/assembly_floorplan_boundary.tsv" \
    "$PROCESSED_ROOT/assembly_fixed_obstacles_normalized.tsv" \
    "$PROCESSED_ROOT/assembly_verified_whitespace_normalized.tsv" \
    "$PROCESSED_ROOT/digital_pg_pair_ranking.tsv" \
