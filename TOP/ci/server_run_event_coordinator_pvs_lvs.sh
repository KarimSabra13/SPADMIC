#!/usr/bin/env bash
# Execute and classify one attributable exact-GDS Event PVS LVS run.
...
PACKAGE_SOURCE="$PACKAGE/netlist/spadmic_event_coordinator.lvs.pg.v"
...
EXPECTED_SOURCE_SHA=f9ec957b23b1a229c7c2ff19309fb7463dfc5cac7e570ad1ca68ad8b08089b27
...
EXPECTED_DENSITY_ROOT=/sim/ksabra/SPADMIC_work/diagnostics/event_pvs_drc_density_execution_20260721_112300
EXPECTED_DENSITY_EXECUTION_HEAD=66ea5eb65de37387a023a77fc4239f1dfab6c6cf
EXPECTED_DENSITY_STATUS_SHA=d7a65bd2b5bef0123120923a7f4104018d14cee6eaf5008d414769413f2759f8
EXPECTED_DENSITY_RUN_SHA=cc4310414f05a394216bcda16f92f884b15e0b980102007386368b362af7f21f
EXPECTED_DENSITY_DEFINES_SHA=6fb64ded8ddc233d1189d0da3cb6e3857ea998dc4ae6c59b7e4db972f89b6e3c
EXPECTED_DENSITY_ANALYSIS_SHA=5a7a929c4600740f4877947c4badef4f6d3469bf458ac81eebe41c42199b3f6e
EXPECTED_DENSITY_INVENTORY_SHA=f27a027627ede5447ea1ae2a2ac5ef7b811bc4a5a5610865a43d0b33dc46b65b
...
TRANSACTION_RESULT=EVENT_EXACT_GDS_PVS_LVS_NOT_EXECUTED
NEXT_GATE=STOP_AND_REVIEW_EVENT_EXACT_GDS_PVS_LVS_FAILURE
...
if [ "$DENSITY_DRC_ROOT" = "MISSING" ]; then
    ...
    RUN_OK=0
elif [ "$DENSITY_DRC_ROOT" != "$EXPECTED_DENSITY_ROOT" ]; then
    echo "STOP_HERE_DO_NOT_CONTINUE: density-DRC diagnostic root is not the accepted Event run"
    RUN_OK=0
fi
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    DIAGNOSTIC_ROOT="$WORK_ROOT/diagnostics/event_pvs_lvs_execution_$EXECUTION_STAMP"
    RUN_ID="event_exact_gds_lvs_$EXECUTION_STAMP"
...
if [ "$RUN_OK" -eq 1 ]; then
    DENSITY_STATUS="$DENSITY_DRC_ROOT/event_pvs_drc_density_execution_status.rpt"
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    for SPEC in \
        "$DENSITY_STATUS|LABEL=SPADMIC_EVENT_PVS_DENSITY_DRC_EXECUTION" \
        "$DENSITY_STATUS|STATUS=PASS" \
        "$DENSITY_STATUS|RESULT=EVENT_PVS_DENSITY_DRC_NONZERO_RULE_DEBT_CLASSIFIED" \
        "$DENSITY_STATUS|EXPECTED_HEAD=$EXPECTED_DENSITY_EXECUTION_HEAD" \
        "$DENSITY_STATUS|ACTUAL_HEAD=$EXPECTED_DENSITY_EXECUTION_HEAD" \
        "$DENSITY_STATUS|TRACKED_DIFF_RC=0" \
        "$DENSITY_STATUS|STAGED_DIFF_RC=0" \
        "$DENSITY_STATUS|DIAGNOSTIC_ROOT=$EXPECTED_DENSITY_ROOT" \
        "$DENSITY_STATUS|PACKAGE=$PACKAGE" \
        "$DENSITY_STATUS|PACKAGE_GDS_SHA256=$EXPECTED_GDS_SHA" \
        "$DENSITY_STATUS|PACKAGE_GATE_RC=0" \
        "$DENSITY_STATUS|PACKAGE_SHA_MANIFEST_RC=0" \
        "$DENSITY_STATUS|SEED_FILE_GATE_RC=0" \
        "$DENSITY_STATUS|SEED_IDENTITY_GATE_RC=0" \
        "$DENSITY_STATUS|SOURCE_REFERENCE_GATE_RC=0" \
        "$DENSITY_STATUS|EVENT_TOP=$EVENT_TOP" \
        "$DENSITY_STATUS|OUTCOME_CLASS=ATTRIBUTABLE_NONZERO_RESULTS" \
        "$DENSITY_STATUS|PVS_WRAPPER_RC=8" \
        "$DENSITY_STATUS|PVS_TOOL_RC=0" \
        "$DENSITY_STATUS|PVS_BASE_DRC_STATUS=PASS" \
        "$DENSITY_STATUS|PVS_DENSITY_DRC_STATUS=FAIL" \
        "$DENSITY_STATUS|EVENT_PVS_BASE_DRC_STATUS=PASS" \
        "$DENSITY_STATUS|EVENT_PVS_DENSITY_DRC_STATUS=FAIL" \
        "$DENSITY_STATUS|DRC_TOTAL_PRIMARY=4" \
        "$DENSITY_STATUS|DRC_TOTAL_EXPANDED=4" \
        "$DENSITY_STATUS|DRC_TOTAL_MATCH_COUNT=3" \
        "$DENSITY_STATUS|REPLAY_CONTRACT_STATUS=PASS" \
        "$DENSITY_STATUS|OUTPUT_ISOLATION_STATUS=PASS" \
        "$DENSITY_STATUS|RUN_FILE_GATE_RC=0" \
        "$DENSITY_STATUS|RUN_AUDIT_GATE_RC=0" \
        "$DENSITY_STATUS|RUN_MANIFEST_RC=0" \
        "$DENSITY_STATUS|RULE_ANALYSIS_RC=0" \
        "$DENSITY_STATUS|RULE_ANALYSIS_STATUS=PASS" \
        "$DENSITY_STATUS|DIAGNOSTIC_COPY_GATE_RC=0" \
        "$DENSITY_STATUS|SOURCE_POST_RECHECK_RC=0" \
        "$DENSITY_STATUS|PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC=0" \
        "$DENSITY_STATUS|EVENT_PVS_BASE_DRC_EXECUTION_AUTHORIZED=NO" \
        "$DENSITY_STATUS|EVENT_PVS_DENSITY_DRC_EXECUTION_AUTHORIZED=YES" \
        "$DENSITY_STATUS|EVENT_PVS_LVS_EXECUTION_AUTHORIZED=NO" \
        "$DENSITY_STATUS|PVS_REPLAY_AUTHORIZED=DENSITY_ONLY" \
        "$DENSITY_STATUS|PVS_EXECUTED=YES" \
        "$DENSITY_STATUS|EVENT_PVS_DENSITY_DRC_EXECUTION_STATUS=PASS" \
        "$DENSITY_STATUS|EVENT_PVS_LVS_STATUS=NOT_RUN" \
        "$DENSITY_STATUS|ASSEMBLY_PHASE=p02_event_control" \
        "$DENSITY_STATUS|ASSEMBLY_INSERTION_AUTHORIZED=NO" \
        "$DENSITY_STATUS|ASSEMBLY_BLOCKED_BY=p00_tx,p01_position" \
        "$DENSITY_STATUS|FULL_TOP_PNR_AUTHORIZED=NO" \
        "$DENSITY_STATUS|BLOCK_PROMOTION_AUTHORIZED=NO" \
        "$DENSITY_STATUS|SIGNOFF_READY=NO" \
        "$DENSITY_STATUS|NEXT_GATE=REVIEW_EVENT_DENSITY_DRC_AND_PREPARE_EXACT_GDS_LVS" \
        ...
        "$DENSITY_RUN_STATUS|PACKAGE=$PACKAGE" \
        "$DENSITY_RUN_STATUS|GDS=$PACKAGE_GDS" \
        "$DENSITY_RUN_STATUS|GDS_SHA256=$EXPECTED_GDS_SHA" \
        "$DENSITY_REPLAY|STATUS=PASS" \
        "$DENSITY_REPLAY|MODE=DRC" \
        ...
        "$DENSITY_REPLAY|GDS=$PACKAGE_GDS" \
        "$DENSITY_ISOLATION|STATUS=PASS" \
        "$DENSITY_ISOLATION|MODE=DRC" \
        "$DENSITY_ISOLATION|LAYOUT_GDS_INPUT=$PACKAGE_GDS" \
...
if [ "$RUN_OK" -eq 1 ]; then
    awk -F '\t' '
        NR == 1 { next }
        {
            ...
            if ($11 != 1 || $12 != "0.000000 0.000000 237.360000 219.520000") bad = 1
        }
        END {
            if (count != 4) bad = 1
...
if [ "$RUN_FILE_GATE_RC" = "0" ]; then
    ...
    for SPEC in \
        ...
        "$RUN_STATUS|LAYOUT_TOP=$EVENT_TOP" \
        "$RUN_STATUS|SOURCE_TOP=$EVENT_TOP" \
        ...
        "$RUN_REPLAY|SOURCE_TOP=$EVENT_TOP" \
...
if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    ...
    for SPEC in \
        "$DENSITY_STATUS|$DIAGNOSTIC_ROOT/source_density_drc/event_pvs_drc_density_execution_status.rpt" \
...
if [ "$OUTCOME_CLASS" = "ATTRIBUTABLE_MATCH" ] && \
   ...
   [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    ...
    TRANSACTION_RESULT=EVENT_PVS_EXACT_GDS_LVS_MATCH_RECORDED
    NEXT_GATE=REVIEW_EVENT_EXACT_GDS_LVS_MATCH_AND_DENSITY_DISPOSITION
     ...
     [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    TRANSACTION_STATUS=PASS
    TRANSACTION_RESULT=EVENT_PVS_EXACT_GDS_LVS_MISMATCH_RECORDED
    NEXT_GATE=CLASSIFY_EVENT_EXACT_GDS_LVS_MISMATCH_NO_RERUN
elif [ "$PVS_EXECUTED" = "YES" ]; then
    TRANSACTION_RESULT=EVENT_PVS_EXACT_GDS_LVS_INFRASTRUCTURE_OR_RESULT_CLASSIFICATION_FAILED
fi

if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    STATUS_REPORT="$DIAGNOSTIC_ROOT/event_pvs_lvs_execution_status.rpt"
    {
        echo "LABEL=SPADMIC_EVENT_PVS_LVS_EXECUTION"
        ...
        echo "SOURCE_DENSITY_DRC_HEAD=$EXPECTED_DENSITY_EXECUTION_HEAD"
        echo "DENSITY_FILE_GATE_RC=$DENSITY_FILE_GATE_RC"
        echo "DENSITY_HASH_GATE_RC=$DENSITY_HASH_GATE_RC"
        echo "DENSITY_MANIFEST_RC=$DENSITY_MANIFEST_RC"
        echo "DENSITY_STATUS_GATE_RC=$DENSITY_STATUS_GATE_RC"
        ...
        echo "LAYOUT_TOP=$EVENT_TOP"
        echo "SOURCE_TOP=$EVENT_TOP"
        echo "RUN_DIR=$RUN_DIR"
        echo "OUTCOME_CLASS=$OUTCOME_CLASS"
        echo "PVS_WRAPPER_RC=$PVS_WRAPPER_RC"
        echo "PVS_TOOL_RC=$PVS_TOOL_RC"
        ...
        echo "EVENT_PVS_BASE_DRC_STATUS=PASS"
        echo "EVENT_PVS_DENSITY_DRC_STATUS=FAIL"
        echo "EVENT_PVS_LVS_EXECUTION_STATUS=$TRANSACTION_STATUS"
        echo "EVENT_PVS_LVS_STATUS=$PVS_LVS_STATUS"
    ...
    echo
    echo "===== EVENT EXACT-GDS PVS LVS EXECUTION STATUS ====="
    cat "$STATUS_REPORT"
