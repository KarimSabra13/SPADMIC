#!/usr/bin/env bash
# Execute and classify one attributable Event base PVS DRC run.
...
EXPECTED_GDS_SHA=837538f219dacc9521f02bc58b3e2e5e587f859b4e70b69a1ca4a3e5fe7b6857
...
EXPECTED_PREFLIGHT_STATUS_SHA=e1769914c88a29744ed55d03360a334e4574f209ed0902fb65ba1f2d39d5aba8
EXPECTED_BASE_STATUS_SHA=1f5a952271218d029b204216454e8c780314f27c4f0c6cd8cc7457e002461206
EXPECTED_DENSITY_STATUS_SHA=fd7b36de4f3bfb0c4d1062c89bc8ada92a8f4c9471a3e318a6e38c68e62f91e1
EXPECTED_REPLAY_SHA=510ce1284530920fbd75445d143e792f0803eaec05f9894e00af336f662dc466
...
EXPECTED_EXTERNAL_REFERENCES_SHA=db7cc5a21531fb7a61ff8aff8c1f22b9f9bea9d55043b92c1c9c21b9f3285658
EXPECTED_GDS_APPLICABILITY_SHA=e67620e9c76681c18bb35918a7ed68a1614466d12c54e895f9a71d5e413d39b7
EXPECTED_OPTION_POLICY_SHA=21dcfba72ddfb989a350d43f5d3a0ea490383c2d940cfb3876664b31f5ba5dc0
EXPECTED_PREFLIGHT_HEAD=9223b07d86273d6e66c11c49691a8d1a2219bfd3
EXPECTED_STAGING_HEAD=0fff3d2afb447f746c69ea946450ff6f5cdd7400

PVS_BIN=/eda/cadence/2023-24/RHELx86/PVS_22.22.000/bin/pvs
DRC_RULE_FILE=/data/pdk/xfab/xh018/cadence/v10_1/pvs/v10_1_1/PVS/xh018_DRC.rul
EXPECTED_DRC_RULE_SHA=0b1ce563da515dd50d17a5e16baa2a2addc10354aa06ab5e1a111b01ed039cb6
STREAM_MAP_FILE=/data/pdk/xfab/xh018/cadence/v10_1/PDK/IC61/v10_1_1/TECH_XH018_HD_1131/pnr_streamout.map
...
TRANSACTION_RESULT=EVENT_BASE_PVS_DRC_NOT_EXECUTED
NEXT_GATE=STOP_AND_REVIEW_EVENT_BASE_PVS_EXECUTION_FAILURE
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    DIAGNOSTIC_ROOT="$WORK_ROOT/diagnostics/event_pvs_drc_base_execution_$EXECUTION_STAMP"
    RUN_ID="event_base_drc_$EXECUTION_STAMP"
...
if [ "$RUN_OK" -eq 1 ]; then
    PREFLIGHT_STATUS="$PREFLIGHT_ROOT/event_pvs_drc_strict_preflight_status.rpt"
    ...
    GDS_APPLICABILITY="$PREFLIGHT_ROOT/gds_layer_applicability/gds_layer_applicability_collector_status.rpt"
    OPTION_POLICY="$PREFLIGHT_ROOT/gds_layer_applicability/event_option_policy_contract.rpt"
    ...
    if [ "$PREFLIGHT_FILE_GATE_RC" -ne 0 ]; then
        echo "STOP_HERE_DO_NOT_CONTINUE: accepted Event strict-preflight evidence is incomplete"
        RUN_OK=0
    fi
fi
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    for SPEC in \
        ...
        "$GDS_APPLICABILITY|$EXPECTED_GDS_APPLICABILITY_SHA" \
        "$OPTION_POLICY|$EXPECTED_OPTION_POLICY_SHA"
    do
        FILE="${SPEC%%|*}"
        EXPECTED_SHA="${SPEC#*|}"
    ...
    if [ "$PREFLIGHT_HASH_GATE_RC" -ne 0 ]; then
        echo "STOP_HERE_DO_NOT_CONTINUE: accepted Event strict-preflight evidence changed"
        RUN_OK=0
    fi
fi
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    if [ "$PREFLIGHT_MANIFEST_RC" -ne 0 ]; then
        ...
        echo "STOP_HERE_DO_NOT_CONTINUE: accepted Event strict-preflight manifest failed"
        RUN_OK=0
    fi
fi
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    for SPEC in \
        ...
        "$PREFLIGHT_STATUS|RESULT=EVENT_BASE_AND_DENSITY_RUN_LOCAL_CONTROLS_MATERIALIZED_WITHOUT_PVS_EXECUTION" \
        "$PREFLIGHT_STATUS|OUTCOME_CLASS=ATTRIBUTABLE_DRY_RUN_CONTROLS" \
        "$PREFLIGHT_STATUS|EXPECTED_HEAD=$EXPECTED_PREFLIGHT_HEAD" \
        "$PREFLIGHT_STATUS|ACTUAL_HEAD=$EXPECTED_PREFLIGHT_HEAD" \
        "$PREFLIGHT_STATUS|SOURCE_STAGING_HEAD=$EXPECTED_STAGING_HEAD" \
        "$PREFLIGHT_STATUS|PACKAGE=$PACKAGE" \
        "$PREFLIGHT_STATUS|PACKAGE_GDS_SHA256=$EXPECTED_GDS_SHA" \
        "$PREFLIGHT_STATUS|EVENT_TOP=$EVENT_TOP" \
        "$PREFLIGHT_STATUS|DRC_RULE_SHA256=$EXPECTED_DRC_RULE_SHA" \
        "$PREFLIGHT_STATUS|STREAM_MAP_SHA256=$EXPECTED_STREAM_MAP_SHA" \
        "$PREFLIGHT_STATUS|PACKAGE_HASH_GATE_RC=0" \
        "$PREFLIGHT_STATUS|PACKAGE_SHA_MANIFEST_RC=0" \
        "$PREFLIGHT_STATUS|HANDOFF_AUDIT_RC=0" \
        "$PREFLIGHT_STATUS|GDS_LAYER_COLLECTOR_RC=0" \
        "$PREFLIGHT_STATUS|GDS_LAYER_APPLICABILITY_GATE_RC=0" \
        "$PREFLIGHT_STATUS|BASE_DRY_RUN_RC=0" \
        "$PREFLIGHT_STATUS|DENSITY_DRY_RUN_RC=0" \
        "$PREFLIGHT_STATUS|RUN_AUDIT_GATE_RC=0" \
        "$PREFLIGHT_STATUS|DIAGNOSTIC_COPY_GATE_RC=0" \
        "$PREFLIGHT_STATUS|SOURCE_POST_RECHECK_RC=0" \
        "$PREFLIGHT_STATUS|SOURCE_STAGING_POST_MANIFEST_RC=0" \
        "$PREFLIGHT_STATUS|PACKAGE_POST_PREFLIGHT_SHA_MANIFEST_RC=0" \
        "$PREFLIGHT_STATUS|EVENT_STRICT_DRY_RUN_PREFLIGHT_STATUS=PASS" \
        "$PREFLIGHT_STATUS|EVENT_PVS_BASE_DRC_EXECUTION_AUTHORIZED=YES" \
        "$PREFLIGHT_STATUS|EVENT_PVS_DENSITY_DRC_EXECUTION_AUTHORIZED=NO" \
        "$PREFLIGHT_STATUS|PVS_REPLAY_AUTHORIZED=BASE_ONLY" \
        "$PREFLIGHT_STATUS|PVS_EXECUTED=NO" \
        "$PREFLIGHT_STATUS|EVENT_PVS_BASE_DRC_STATUS=NOT_RUN" \
        "$PREFLIGHT_STATUS|EVENT_PVS_DENSITY_DRC_STATUS=NOT_RUN" \
        "$PREFLIGHT_STATUS|EVENT_PVS_LVS_STATUS=NOT_RUN" \
        "$PREFLIGHT_STATUS|NEXT_GATE=RUN_EVENT_PVS_BASE_DRC_ON_EXACT_STAGED_GDS" \
        ...
        "$BASE_STATUS|PACKAGE=$PACKAGE" \
        "$BASE_STATUS|GDS=$PACKAGE_GDS" \
        ...
        "$DENSITY_STATUS|PACKAGE=$PACKAGE" \
        "$DENSITY_STATUS|GDS=$PACKAGE_GDS" \
        ...
        "$BASE_REPLAY|LAYOUT_TOP=$EVENT_TOP" \
        "$DENSITY_REPLAY|STATUS=PASS" \
        "$DENSITY_REPLAY|EXECUTION_DIRECTORY_STATUS=PASS" \
        "$DENSITY_REPLAY|OUTPUT_ISOLATION_STATUS=PASS" \
        "$DENSITY_REPLAY|LAYOUT_TOP=$EVENT_TOP" \
        "$BASE_DEFINES|UNDEFINE=DENSITY|OCCURRENCES=1" \
        "$DENSITY_DEFINES|DEFINE=DENSITY|OCCURRENCES=1" \
        "$GDS_APPLICABILITY|COLLECTOR_STATUS=PASS" \
        "$GDS_APPLICABILITY|GDS_SHA256=$EXPECTED_GDS_SHA" \
        "$GDS_APPLICABILITY|TOP_STRUCTURE=$EVENT_TOP" \
        "$GDS_APPLICABILITY|PAD_REACHABLE_GEOMETRY_ELEMENT_COUNT=0" \
        "$GDS_APPLICABILITY|PIMIDE_REACHABLE_GEOMETRY_ELEMENT_COUNT=0" \
        "$GDS_APPLICABILITY|NOPIM_REACHABLE_GEOMETRY_ELEMENT_COUNT=0" \
        "$GDS_APPLICABILITY|PIMIDE_EVENT_APPLICABILITY_STATUS=NOT_APPLICABLE_NO_REACHABLE_PAD_OR_PIMIDE_GEOMETRY" \
        "$OPTION_POLICY|DEFAULT_RULE_SET_SELECTION_STATUS=PASS" \
        "$OPTION_POLICY|DENSITY_POLICY=BASE_DRC_PLUS_SEPARATE_DENSITY_DRC" \
        "$OPTION_POLICY|PIMIDE_STATE=UNDEFINED" \
        "$OPTION_POLICY|POPPING_STATE=UNDEFINED" \
        "$OPTION_POLICY|DUMMY_FILL_STATE=UNDEFINED" \
        "$OPTION_POLICY|VAR_ANT_RATIO_STATE=DEFINED"
    do
        FILE="${SPEC%%|*}"
        EXPECTED_LINE="${SPEC#*|}"
    ...
    if [ "$PREFLIGHT_STATUS_GATE_RC" -ne 0 ]; then
        echo "STOP_HERE_DO_NOT_CONTINUE: accepted Event preflight does not authorize base execution"
        RUN_OK=0
    fi
fi
...
if [ "$RUN_FILE_GATE_RC" = "0" ]; then
    ...
    for SPEC in \
        ...
        "$RUN_REPLAY|LAYOUT_TOP=$EVENT_TOP" \
...
if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    ...
    for SPEC in \
        "$PREFLIGHT_STATUS|$DIAGNOSTIC_ROOT/source_preflight/event_pvs_drc_strict_preflight_status.rpt" \
        ...
        "$GDS_APPLICABILITY|$DIAGNOSTIC_ROOT/source_preflight/gds_layer_applicability_collector_status.rpt" \
        "$OPTION_POLICY|$DIAGNOSTIC_ROOT/source_preflight/event_option_policy_contract.rpt" \
...
if [ "$OUTCOME_CLASS" = "ATTRIBUTABLE_ZERO_RESULTS" ] && \
   ...
   [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    ...
    TRANSACTION_RESULT=EVENT_PVS_BASE_DRC_ZERO_RESULTS_RECORDED
    NEXT_GATE=REVIEW_EVENT_BASE_DRC_AND_PREPARE_DENSITY_EXECUTION
     ...
     [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    TRANSACTION_STATUS=PASS
    TRANSACTION_RESULT=EVENT_PVS_BASE_DRC_NONZERO_RULE_DEBT_CLASSIFIED
    NEXT_GATE=REVIEW_EVENT_BASE_DRC_AND_PREPARE_DENSITY_EXECUTION
elif [ "$OUTCOME_CLASS" = "ATTRIBUTABLE_NONZERO_RESULTS" ]; then
    TRANSACTION_RESULT=EVENT_PVS_BASE_DRC_NONZERO_RULE_ANALYSIS_FAILED
    NEXT_GATE=STOP_AND_REVIEW_EVENT_BASE_RULE_ANALYSIS_FAILURE
elif [ "$PVS_EXECUTED" = "YES" ]; then
    TRANSACTION_RESULT=EVENT_PVS_BASE_DRC_INFRASTRUCTURE_OR_RESULT_CLASSIFICATION_FAILED
fi

if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    STATUS_REPORT="$DIAGNOSTIC_ROOT/event_pvs_drc_base_execution_status.rpt"
    {
        echo "LABEL=SPADMIC_EVENT_PVS_BASE_DRC_EXECUTION"
        ...
        echo "SOURCE_STRICT_PREFLIGHT_HEAD=$EXPECTED_PREFLIGHT_HEAD"
        echo "PREFLIGHT_FILE_GATE_RC=$PREFLIGHT_FILE_GATE_RC"
        echo "PREFLIGHT_HASH_GATE_RC=$PREFLIGHT_HASH_GATE_RC"
        echo "PREFLIGHT_MANIFEST_RC=$PREFLIGHT_MANIFEST_RC"
        echo "PREFLIGHT_STATUS_GATE_RC=$PREFLIGHT_STATUS_GATE_RC"
        ...
        echo "PVS_TOOL_RC=$PVS_TOOL_RC"
        echo "PVS_BASE_DRC_STATUS=$PVS_BASE_DRC_STATUS"
        echo "EVENT_PVS_BASE_DRC_STATUS=$PVS_BASE_DRC_STATUS"
        echo "DRC_TOTAL_PRIMARY=$DRC_TOTAL_PRIMARY"
        echo "DRC_TOTAL_EXPANDED=$DRC_TOTAL_EXPANDED"
        ...
        echo "SOURCE_POST_RECHECK_RC=$SOURCE_POST_RECHECK_RC"
        echo "PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC=$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC"
        echo "EVENT_PVS_BASE_DRC_EXECUTION_AUTHORIZED=$PVS_BASE_DRC_EXECUTION_AUTHORIZED"
        ...
        echo "EVENT_PVS_BASE_DRC_EXECUTION_STATUS=$TRANSACTION_STATUS"
    ...
    echo
    echo "===== EVENT BASE PVS DRC EXECUTION STATUS ====="
    cat "$STATUS_REPORT"
    if [ -r "$DIAGNOSTIC_ROOT/rule_analysis/pvs_drc_analysis_status.rpt" ]; then
        echo
        echo "===== EVENT BASE PVS DRC RULE ANALYSIS STATUS ====="
