#!/usr/bin/env bash
# Execute and classify one attributable Event density PVS DRC run.
...
EXPECTED_BASE_EXECUTION_STATUS_SHA=616f4c450b41eb0ed626a4cb73b6e78cf8076f6b0671d0d058200e8bb47688ea
EXPECTED_BASE_RUN_STATUS_SHA=b829aedce7251ed8a390efd14694659f6dae1add311a2c4a5d7d0f5c55cda11a
EXPECTED_BASE_REPLAY_SHA=510ce1284530920fbd75445d143e792f0803eaec05f9894e00af336f662dc466
EXPECTED_BASE_ISOLATION_SHA=d27f4d4297749b019cdc0c88be331706452072521cd0e836e7c84787f4ddeea1
EXPECTED_BASE_DEFINES_SHA=1f8ef07f31201a170d0f770d26596c031821f4c72d58b3ab4517b90f4f837dfa
EXPECTED_BASE_REFERENCES_SHA=db7cc5a21531fb7a61ff8aff8c1f22b9f9bea9d55043b92c1c9c21b9f3285658
EXPECTED_BASE_DRC_ROOT=/sim/ksabra/SPADMIC_work/diagnostics/event_pvs_drc_base_execution_20260721_110404
EXPECTED_BASE_EXECUTION_HEAD=e0325d688a73b261742dc70097b1059aba8e035b
...
TRANSACTION_RESULT=EVENT_DENSITY_PVS_DRC_NOT_EXECUTED
NEXT_GATE=STOP_AND_REVIEW_EVENT_DENSITY_PVS_EXECUTION_FAILURE
...
if [ "$BASE_DRC_ROOT" = "MISSING" ]; then
    ...
    RUN_OK=0
elif [ "$BASE_DRC_ROOT" != "$EXPECTED_BASE_DRC_ROOT" ]; then
    echo "STOP_HERE_DO_NOT_CONTINUE: base-DRC diagnostic root is not the accepted Event run"
    RUN_OK=0
fi
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    DIAGNOSTIC_ROOT="$WORK_ROOT/diagnostics/event_pvs_drc_density_execution_$EXECUTION_STAMP"
    RUN_ID="event_density_drc_$EXECUTION_STAMP"
...
if [ "$RUN_OK" -eq 1 ]; then
    BASE_EXECUTION_STATUS="$BASE_DRC_ROOT/event_pvs_drc_base_execution_status.rpt"
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    for SPEC in \
        "$BASE_EXECUTION_STATUS|LABEL=SPADMIC_EVENT_PVS_BASE_DRC_EXECUTION" \
        "$BASE_EXECUTION_STATUS|STATUS=PASS" \
        "$BASE_EXECUTION_STATUS|RESULT=EVENT_PVS_BASE_DRC_ZERO_RESULTS_RECORDED" \
        "$BASE_EXECUTION_STATUS|EXPECTED_HEAD=$EXPECTED_BASE_EXECUTION_HEAD" \
        "$BASE_EXECUTION_STATUS|ACTUAL_HEAD=$EXPECTED_BASE_EXECUTION_HEAD" \
        "$BASE_EXECUTION_STATUS|TRACKED_DIFF_RC=0" \
        "$BASE_EXECUTION_STATUS|STAGED_DIFF_RC=0" \
        "$BASE_EXECUTION_STATUS|SOURCE_STRICT_PREFLIGHT_HEAD=$EXPECTED_PREFLIGHT_HEAD" \
        "$BASE_EXECUTION_STATUS|PREFLIGHT_FILE_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|PREFLIGHT_HASH_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|PREFLIGHT_MANIFEST_RC=0" \
        "$BASE_EXECUTION_STATUS|PREFLIGHT_STATUS_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|PACKAGE=$PACKAGE" \
        "$BASE_EXECUTION_STATUS|PACKAGE_GDS_SHA256=$EXPECTED_GDS_SHA" \
        "$BASE_EXECUTION_STATUS|PACKAGE_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|PACKAGE_SHA_MANIFEST_RC=0" \
        "$BASE_EXECUTION_STATUS|SEED_FILE_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|SEED_IDENTITY_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|SOURCE_REFERENCE_GATE_RC=0" \
        "$BASE_EXECUTION_STATUS|EVENT_TOP=$EVENT_TOP" \
        "$BASE_EXECUTION_STATUS|DIAGNOSTIC_ROOT=$EXPECTED_BASE_DRC_ROOT" \
        ...
        "$BASE_EXECUTION_STATUS|EVENT_PVS_BASE_DRC_STATUS=PASS" \
        "$BASE_EXECUTION_STATUS|DRC_TOTAL_PRIMARY=0" \
        "$BASE_EXECUTION_STATUS|DRC_TOTAL_EXPANDED=0" \
        "$BASE_EXECUTION_STATUS|DRC_TOTAL_MATCH_COUNT=3" \
        ...
        "$BASE_EXECUTION_STATUS|EVENT_PVS_BASE_DRC_EXECUTION_AUTHORIZED=YES" \
        "$BASE_EXECUTION_STATUS|EVENT_PVS_DENSITY_DRC_EXECUTION_AUTHORIZED=NO" \
        "$BASE_EXECUTION_STATUS|PVS_REPLAY_AUTHORIZED=BASE_ONLY" \
        "$BASE_EXECUTION_STATUS|PVS_EXECUTED=YES" \
        "$BASE_EXECUTION_STATUS|EVENT_PVS_BASE_DRC_EXECUTION_STATUS=PASS" \
        "$BASE_EXECUTION_STATUS|EVENT_PVS_DENSITY_DRC_STATUS=NOT_RUN" \
        "$BASE_EXECUTION_STATUS|EVENT_PVS_LVS_STATUS=NOT_RUN" \
        "$BASE_EXECUTION_STATUS|ASSEMBLY_PHASE=p02_event_control" \
        "$BASE_EXECUTION_STATUS|ASSEMBLY_INSERTION_AUTHORIZED=NO" \
        "$BASE_EXECUTION_STATUS|ASSEMBLY_BLOCKED_BY=p00_tx,p01_position" \
        "$BASE_EXECUTION_STATUS|FULL_TOP_PNR_AUTHORIZED=NO" \
        "$BASE_EXECUTION_STATUS|BLOCK_PROMOTION_AUTHORIZED=NO" \
        "$BASE_EXECUTION_STATUS|SIGNOFF_READY=NO" \
        "$BASE_EXECUTION_STATUS|NEXT_GATE=REVIEW_EVENT_BASE_DRC_AND_PREPARE_DENSITY_EXECUTION" \
        "$BASE_RUN_STATUS|PVS_RC=0" \
        "$BASE_RUN_STATUS|PVS_DRC_STATUS=PASS" \
        "$BASE_RUN_STATUS|DRC_TOTAL_MATCH_COUNT=3" \
...
if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    ...
    for SPEC in \
        "$BASE_EXECUTION_STATUS|$DIAGNOSTIC_ROOT/source_base_drc/event_pvs_drc_base_execution_status.rpt" \
...
if [ "$OUTCOME_CLASS" = "ATTRIBUTABLE_ZERO_RESULTS" ] && \
   ...
   [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    ...
    TRANSACTION_RESULT=EVENT_PVS_DENSITY_DRC_ZERO_RESULTS_RECORDED
    NEXT_GATE=REVIEW_EVENT_DENSITY_DRC_AND_PREPARE_EXACT_GDS_LVS
     ...
     [ "$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC" = "0" ]; then
    TRANSACTION_STATUS=PASS
    TRANSACTION_RESULT=EVENT_PVS_DENSITY_DRC_NONZERO_RULE_DEBT_CLASSIFIED
    NEXT_GATE=REVIEW_EVENT_DENSITY_DRC_AND_PREPARE_EXACT_GDS_LVS
elif [ "$OUTCOME_CLASS" = "ATTRIBUTABLE_NONZERO_RESULTS" ]; then
    TRANSACTION_RESULT=EVENT_PVS_DENSITY_DRC_NONZERO_RULE_ANALYSIS_FAILED
    NEXT_GATE=STOP_AND_REVIEW_EVENT_DENSITY_RULE_ANALYSIS_FAILURE
elif [ "$PVS_EXECUTED" = "YES" ]; then
    TRANSACTION_RESULT=EVENT_PVS_DENSITY_DRC_INFRASTRUCTURE_OR_RESULT_CLASSIFICATION_FAILED
fi

if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    STATUS_REPORT="$DIAGNOSTIC_ROOT/event_pvs_drc_density_execution_status.rpt"
    {
        echo "LABEL=SPADMIC_EVENT_PVS_DENSITY_DRC_EXECUTION"
        ...
        echo "SOURCE_BASE_DRC_HEAD=$EXPECTED_BASE_EXECUTION_HEAD"
        echo "BASE_FILE_GATE_RC=$BASE_FILE_GATE_RC"
        echo "BASE_HASH_GATE_RC=$BASE_HASH_GATE_RC"
        echo "BASE_MANIFEST_RC=$BASE_MANIFEST_RC"
        echo "BASE_STATUS_GATE_RC=$BASE_STATUS_GATE_RC"
        ...
        echo "EVENT_PVS_DENSITY_DRC_STATUS=$PVS_DENSITY_DRC_STATUS"
        echo "DRC_TOTAL_PRIMARY=$DRC_TOTAL_PRIMARY"
        echo "DRC_TOTAL_EXPANDED=$DRC_TOTAL_EXPANDED"
        ...
        echo "SOURCE_POST_RECHECK_RC=$SOURCE_POST_RECHECK_RC"
        echo "PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC=$PACKAGE_POST_EXECUTION_SHA_MANIFEST_RC"
        echo "EVENT_PVS_BASE_DRC_EXECUTION_AUTHORIZED=NO"
        echo "EVENT_PVS_DENSITY_DRC_EXECUTION_AUTHORIZED=$PVS_DENSITY_DRC_EXECUTION_AUTHORIZED"
        echo "EVENT_PVS_LVS_EXECUTION_AUTHORIZED=NO"
        echo "PVS_REPLAY_AUTHORIZED=$PVS_REPLAY_AUTHORIZED"
        echo "PVS_EXECUTED=$PVS_EXECUTED"
        echo "EVENT_PVS_DENSITY_DRC_EXECUTION_STATUS=$TRANSACTION_STATUS"
    ...
    echo
    echo "===== EVENT DENSITY PVS DRC EXECUTION STATUS ====="
    cat "$STATUS_REPORT"
    if [ -r "$DIAGNOSTIC_ROOT/rule_analysis/pvs_drc_analysis_status.rpt" ]; then
        echo
        echo "===== EVENT DENSITY PVS DRC RULE ANALYSIS STATUS ====="
