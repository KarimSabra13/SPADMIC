...
if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    ...
    do
        ...
        VARIANT_REMAINDER="${VARIANT_SPEC#*|}"
        DESTINATION_DIR="${VARIANT_REMAINDER%%|*}"
        VARIANT_DRY_RUN_RC="${VARIANT_REMAINDER#*|}"
        for NAME in \
            pvs_drc_status.rpt \
            replay_contract_status.rpt \
            output_isolation.rpt \
...
if [ "$DIAGNOSTIC_ROOT" != "UNKNOWN" ]; then
    if [ "$RUN_OK" -eq 1 ]; then
        FINAL_STATUS=PASS
        FINAL_RESULT=BASE_AND_DENSITY_RUN_LOCAL_CONTROLS_MATERIALIZED_WITHOUT_PVS_EXECUTION
        PREFLIGHT_STATUS=PASS
        NEXT_GATE=RETURN_STRICT_DRY_RUN_CONTROLS_FOR_BASE_EXECUTION_AUTHORIZATION
    else
        FINAL_STATUS=FAIL
        FINAL_RESULT=STRICT_DRY_RUN_PREFLIGHT_INCOMPLETE
