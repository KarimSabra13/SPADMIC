...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    grep -Eq '^[[:space:]]*techLib[[:space:]]+"TECH_XH018_HD"[[:space:]]*$' \
        "$PRIMARY_SEED/pipo1.setup"
    TECHLIB_CONTRACT_RC=$?
    echo "CONTRACT_REGEX_RC=$TECHLIB_CONTRACT_RC FILE=$PRIMARY_SEED/pipo1.setup EXPECTED_TECHLIB=TECH_XH018_HD" \
        >>"$CONTRACT_REPORT"

    if [ "$TECHLIB_CONTRACT_RC" -ne 0 ]; then
        PRIMARY_EXECUTABLE_CONTRACT_GATE_RC=1
    fi
    ...
    PREPROCESSOR_REPORT="$DIAGNOSTIC_ROOT/primary_pvsdrcctl_preprocessor_directives.rpt"
    {
        echo "LABEL=SPADMIC_POSITION_PVS_DRC_SEED_PREPROCESSOR_DIRECTIVES"
        grep -nE '^[[:space:]]*#(UN)?DEFINE[[:space:]]+' \
            "$PRIMARY_SEED/pvsdrcctl"
    } >"$PREPROCESSOR_REPORT"

    NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT="$(
        grep -E '^[0-9]+:' "$PREPROCESSOR_REPORT" 2>/dev/null |
        grep -Evc '#(UN)?DEFINE[[:space:]]+DENSITY([[:space:]]|$)'
    )"

    PREPROCESSOR_DIRECTIVE_REVIEW_STATUS=PASS
    if [ "$NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT" -ne 0 ]; then
    ...
    echo "NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT=$NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT" \
        >>"$CONTRACT_REPORT"
    echo "PREPROCESSOR_DIRECTIVE_REVIEW_STATUS=$PREPROCESSOR_DIRECTIVE_REVIEW_STATUS" \
        >>"$CONTRACT_REPORT"
...
if [ "$RUN_OK" -eq 1 ]; then
    ...
    if [ "$PREPROCESSOR_DIRECTIVE_REVIEW_STATUS" != "PASS" ]; then
    ...
    STATUS_REPORT="$DIAGNOSTIC_ROOT/seed_review_status.rpt"
    {
        ...
        echo "PREPROCESSOR_DIRECTIVE_COUNT=$PREPROCESSOR_DIRECTIVE_COUNT"
        echo "NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT=$NON_DENSITY_PREPROCESSOR_DIRECTIVE_COUNT"
        echo "PREPROCESSOR_DIRECTIVE_REVIEW_STATUS=$PREPROCESSOR_DIRECTIVE_REVIEW_STATUS"
        echo "AUTOMATED_CONTROL_RISK_SCAN_STATUS=$AUTOMATED_CONTROL_RISK_SCAN_STATUS"
        echo "EXECUTABLE_RISK_LINE_COUNT=$EXECUTABLE_RISK_LINE_COUNT"
