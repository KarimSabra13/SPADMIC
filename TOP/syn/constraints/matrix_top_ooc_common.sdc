...
# Treat clk_sys and clk_cfg_40m as separate for now. The final PLL relationship
# and macro timing handoff must replace this placeholder before signoff.
if {[llength [get_clocks clk_sys -quiet]] > 0 && [llength [get_clocks clk_cfg_40m -quiet]] > 0} {
  set_clock_groups -asynchronous -group [get_clocks clk_sys] -group [get_clocks clk_cfg_40m]
}

if {[llength [get_clocks clk_sys -quiet]] > 0 && [llength [get_clocks clk_ref_40m -quiet]] > 0} {
