`timescale 1ps/1ps
`default_nettype none

module tb_spadmic_top_matrix_v1_skew_campaign;
  import spadmic_pkg::*;

  localparam int CLK_SYS_PERIOD   = 6250;
  localparam int CLK_40M_PERIOD   = 25000;
  localparam int I2C_HALF_PERIOD  = 200_000;
  localparam int I2C_SAMPLE_DELAY = I2C_HALF_PERIOD/2;

  logic clk_sys;
  logic clk_ref_40m;
  logic clk_cfg_40m;
  logic async_rst_n;
  logic i2c_scl_drv;
  logic i2c_sda_master_low;
  wire i2c_scl = i2c_scl_drv;
  wire i2c_sda;
  wire i2c_sda_oe;
  logic [63:0] R;
  logic [63:0] Y;
  logic [63:0] B;
  wire [63:0] Rz;
  wire [63:0] Yz;
  wire [63:0] Bz;
  wire [43:0] matrix_din;
  wire [43:0] matrix_cin;
  logic [43:0] matrix_dout;
  logic [43:0] matrix_cout;
  wire [15:0] ddr_data_l;
  wire [15:0] ddr_data_h;
  wire ddr_pair_valid;
  wire ddr_clk;
  int pass_count;
  int fail_count;

  assign i2c_sda = (i2c_sda_master_low | i2c_sda_oe) ? 1'b0 : 1'b1;

  initial clk_sys = 1'b0;
  always #(CLK_SYS_PERIOD/2) clk_sys = ~clk_sys;

  initial clk_ref_40m = 1'b0;
  always #(CLK_40M_PERIOD/2) clk_ref_40m = ~clk_ref_40m;

  initial clk_cfg_40m = 1'b0;
  always #(CLK_40M_PERIOD/2) clk_cfg_40m = ~clk_cfg_40m;

  spadmic_top_matrix_v1 dut (
    .clk_sys(clk_sys),
    .clk_ref_40m(clk_ref_40m),
    .clk_cfg_40m(clk_cfg_40m),
    .async_rst_n(async_rst_n),
    .i2c_scl_i(i2c_scl),
    .i2c_sda_i(i2c_sda),
    .i2c_sda_oe_o(i2c_sda_oe),
    .R_i(R),
    .Y_i(Y),
    .B_i(B),
    .Rz_o(Rz),
    .Yz_o(Yz),
    .Bz_o(Bz),
    .matrix_din_o(matrix_din),
    .matrix_cin_o(matrix_cin),
    .matrix_dout_i(matrix_dout),
    .matrix_cout_i(matrix_cout),
    .cal_r_start_async_i(1'b0),
    .cal_r_stop_async_i(1'b0),
    .cal_y_start_async_i(1'b0),
    .cal_y_stop_async_i(1'b0),
    .cal_b_start_async_i(1'b0),
    .cal_b_stop_async_i(1'b0),
    .ddr_data_l_o(ddr_data_l),
    .ddr_data_h_o(ddr_data_h),
    .ddr_pair_valid_o(ddr_pair_valid),
    .ddr_clk_o(ddr_clk)
  );

  `include "TOP/tb/spadmic_top_matrix_v1_i2c_tasks.svh"

  task automatic check(input string label, input logic cond);
    if (cond) begin
      $display("[PASS] %s", label);
      pass_count++;
    end else begin
      $display("[FAIL] %s", label);
      fail_count++;
    end
  endtask

  task automatic drive_axis(input int axis, input logic value);
    begin
      case (axis)
        0: R[3] = value;
        1: Y[5] = value;
        2: B[7] = value;
        default: ;
      endcase
    end
  endtask

  function automatic logic [2:0] axis_mask(input int axis);
    case (axis)
      0: axis_mask = 3'b001;
      1: axis_mask = 3'b010;
      2: axis_mask = 3'b100;
      default: axis_mask = 3'b000;
    endcase
  endfunction

  function automatic string axis_name(input int axis);
    case (axis)
      0: axis_name = "R";
      1: axis_name = "Y";
      2: axis_name = "B";
      default: axis_name = "?";
    endcase
  endfunction

  task automatic wait_start_seen(input logic [2:0] mask, input string label);
    int guard;
    begin
      guard = 0;
      while (((dut.tdc_start_seen_q & mask) != mask) && guard < 200) begin
        guard++;
        @(posedge clk_sys);
      end
      check(label, ((dut.tdc_start_seen_q & mask) == mask));
    end
  endtask

  task automatic wait_safe_idle(input string label);
    int guard;
    begin
      guard = 0;
      while (!dut.safe_idle && guard < 50_000) begin
        guard++;
        @(posedge clk_sys);
      end
      check(label, dut.safe_idle);
    end
  endtask

  task automatic run_order(
    input int first_axis,
    input int second_axis,
    input int third_axis,
    input int gap1_cycles,
    input int gap2_cycles
  );
    logic [2:0] seen_mask;
    string label;
    begin
      label = {axis_name(first_axis), "->", axis_name(second_axis), "->",
               axis_name(third_axis)};
      wait_safe_idle({label, " starts from safe idle"});
      check({label, " resources ready before first edge"}, dut.pre_event_resources_ready);

      drive_axis(first_axis, 1'b1);
      #1;
      check({label, " first axis reaches only its own START gate"},
            dut.tdc_start_async_to_core == axis_mask(first_axis));
      seen_mask = axis_mask(first_axis);
      wait_start_seen(seen_mask, {label, " first start seen"});
      repeat (gap1_cycles) @(posedge clk_sys);

      drive_axis(second_axis, 1'b1);
      #1;
      check({label, " second axis still reaches START after event open"},
            (dut.tdc_start_async_to_core & axis_mask(second_axis)) != 3'b000);
      seen_mask |= axis_mask(second_axis);
      wait_start_seen(seen_mask, {label, " second start seen"});
      repeat (gap2_cycles) @(posedge clk_sys);

      drive_axis(third_axis, 1'b1);
      #1;
      check({label, " third axis still reaches START after skew"},
            (dut.tdc_start_async_to_core & axis_mask(third_axis)) != 3'b000);
      seen_mask |= axis_mask(third_axis);
      wait_start_seen(seen_mask, {label, " all starts seen"});

      wait (Rz != {64{1'b1}});
      check({label, " reset waits for all required starts"}, dut.tdc_start_seen_q == 3'b111);
      repeat (4) @(posedge clk_sys);
      R = '0;
      Y = '0;
      B = '0;
      wait_safe_idle({label, " drains and rearms"});
    end
  endtask

  initial begin
    pass_count = 0;
    fail_count = 0;
    async_rst_n = 1'b0;
    i2c_scl_drv = 1'b1;
    i2c_sda_master_low = 1'b0;
    R = '0;
    Y = '0;
    B = '0;
    matrix_dout = '0;
    matrix_cout = '0;

    repeat (8) @(posedge clk_sys);
    async_rst_n = 1'b1;
    repeat (12) @(posedge clk_sys);

    i2c_write_csr(SPADMIC_CSR_MATRIX_RESET_CTRL, 32'h0001_0003);
    i2c_write_csr(SPADMIC_CSR_MTOP_CTRL_REQUEST,
                  {24'h0, 1'b1, 3'b111, SPADMIC_MODE_TDC_ONLY, 1'b1});

    run_order(0, 1, 2, 1, 2);
    run_order(0, 2, 1, 2, 4);
    run_order(1, 0, 2, 3, 1);
    run_order(1, 2, 0, 4, 3);
    run_order(2, 0, 1, 5, 2);
    run_order(2, 1, 0, 8, 5);

    if (fail_count != 0)
      $fatal(1, "tb_spadmic_top_matrix_v1_skew_campaign: %0d failures", fail_count);

    $display("tb_spadmic_top_matrix_v1_skew_campaign: %0d pass / %0d fail",
             pass_count, fail_count);
    $finish;
  end

  initial begin
    #8_000_000_000;
    $fatal(1, "tb_spadmic_top_matrix_v1_skew_campaign: TIMEOUT");
  end
endmodule

`default_nettype wire
