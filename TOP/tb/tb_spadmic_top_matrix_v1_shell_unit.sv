...
module tb_spadmic_top_matrix_v1_shell_unit;
  ...
  initial begin
    ...
    i2c_read_csr({4'h0, SPADMIC_CSR_GLOBAL_ID}, rd);
    check("I2C reads matrix-top ID", rd == 32'h5350_4D54);

    i2c_write_csr({4'h0, SPADMIC_CSR_MATRIX_RESET_CTRL}, 32'h0001_0003);
    i2c_read_csr({4'h0, SPADMIC_CSR_MATRIX_RESET_CTRL}, rd);
    check("I2C writes reset width and auto reset", rd[16] && rd[15:0] == 16'd3);

    i2c_write_csr({4'h0, SPADMIC_CSR_MTOP_CTRL_REQUEST},
                  {24'h0, 1'b1, 3'b111, SPADMIC_MODE_POSITION_ONLY, 1'b1});
    i2c_read_csr({4'h0, SPADMIC_CSR_MTOP_CTRL_ACTIVE}, rd);
    ...
    i2c_read_csr({4'h0, SPADMIC_CSR_TX_STATUS}, rd);
    check("position-only event drains DDR16 pairer", rd[0] && !rd[1]);
    i2c_read_csr({4'h0, SPADMIC_CSR_MTOP_STATUS}, rd);
    ...
    i2c_write_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_COL}, 32'd7);
    i2c_write_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_WDATA_LO}, 32'h89AB_CDEF);
    i2c_write_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_WDATA_HI}, 32'h0123_4567);
    i2c_write_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_CMD}, {28'h0, OP_WRITE_COLUMN_64, 1'b1});
    repeat (300) @(posedge clk_sys);
    i2c_read_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_STATUS}, rd);
    check("I2C observes matrix cfg valid", rd[8] && rd[7] && !rd[2]);
    i2c_read_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_RDATA_LO}, rd);
    check("I2C reads matrix cfg low readback", rd == 32'h89AB_CDEF);
    i2c_read_csr({4'h0, SPADMIC_CSR_MATRIX_CFG_RDATA_HI}, rd);
    ...
    i2c_read_csr({4'h0, SPADMIC_CSR_MATRIX_EVENT_STATUS}, rd);
