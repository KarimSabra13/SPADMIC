...
module tb_spadmic_tx_egress_core_unit;
  import mptdc_pkg::*;
  import spadmic_pkg::*;
  ...
  initial begin
    ...
    #1;
    check("core starts idle", bundle_idle && output_fifo_empty && ddr_empty);
    ...
    check("core completed all source packets", completed_packet_mask == 4'b1111);
    check("core emitted four DDRs2-valid pairs", pair_count == 4);
    check("first low lanes carry R header", tdc_header_source_id(lanes_l[0][15:0]) == TDC_ID_X);
    check("first high lanes carry common EOC", lanes_h[0][15:0] == {2'b11, event_id});
    check("position pair low lanes carry position header",
          lanes_l[3][15:0] == spadmic_pos_raw_header_word(3'b111));
    ...
    check("core does not pad even bundle", !ddr_padded);
    ...
    if (fail_count != 0)
      $fatal(1, "tb_spadmic_tx_egress_core_unit: %0d failures", fail_count);

    $display("tb_spadmic_tx_egress_core_unit: %0d pass / %0d fail",
  ...
  initial begin
    #2_000_000;
    $fatal(1, "tb_spadmic_tx_egress_core_unit: TIMEOUT");
  end
endmodule
