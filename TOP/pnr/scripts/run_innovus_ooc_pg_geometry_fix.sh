#!/usr/bin/env bash
# Deterministic P02-R2 PG geometry fix for the routed tx_ddr_strip checkpoint.
set -u -o pipefail
...
if [[ "$BLOCK" != "tx_ddr_strip" ]]; then
  echo "ERROR: P02-R2 is approved only for tx_ddr_strip" >&2
  exit 6
fi
...

{
  ...
  echo "POLICY=RESTORE_P01_EXPLICIT_PG_GEOMETRY_NO_SIGNAL_ROUTE"
