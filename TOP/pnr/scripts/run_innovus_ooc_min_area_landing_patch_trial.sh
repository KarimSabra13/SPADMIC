...
Usage:
  TOP/pnr/scripts/run_innovus_ooc_min_area_landing_patch_trial.sh \
    <source-root> <step20-analysis> [trial-id] [top-module] [checkpoint]
...
main() {
  ...
  if [[ ! -d "$source_root" || ! -r "$analysis" ]]; then
    echo "ERROR: source root or Step 20 analysis is missing" >&2
    echo "SOURCE_ROOT=$source_root" >&2
    echo "STEP20_ANALYSIS=$analysis" >&2
    return 6
  fi
  ...
  export SPADMIC_MIN_AREA_LANDING_STEP20_ANALYSIS="$analysis"
  ...
  analysis_sha="$(sha256sum "$analysis" 2>/dev/null | awk '{print $1}')"
  {
    ...
    echo "STEP20_ANALYSIS=$analysis"
    echo "STEP20_ANALYSIS_SHA256=${analysis_sha:-MISSING}"
    echo "TOP_MODULE=$top"
    echo "TRIAL_ROOT=$trial_root"
    echo "HEAD=$(git -C "$script_dir/../../.." rev-parse HEAD 2>/dev/null)"
    echo "POLICY=ONE_FRESH_PROCESS_ONE_RESTORE_SIX_BOUNDED_MET1_LANDING_EXTENSIONS"
