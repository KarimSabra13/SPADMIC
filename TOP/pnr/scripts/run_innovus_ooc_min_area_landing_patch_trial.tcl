...
set step20_analysis [lp_env SPADMIC_MIN_AREA_LANDING_STEP20_ANALYSIS]
...
set status(STEP20_ANALYSIS) $step20_analysis

array set analysis_values [lp_read_kv $step20_analysis]
...
foreach key [array names expected_analysis] {
    ...
    if {$actual ne $expected_analysis($key)} {
        lp_abort STEP20_ANALYSIS_NOT_ACCEPTED "$key=$actual expected=$expected_analysis($key)"
    }
}
...
# net marker-box center-y extension-x source-Q source-Q-x source-Q-y
set patch_contract [list \
    [list n_9696 {719.69 158.62 720.07 158.90} 719.88 158.76 719.32 g14627__2802/Q 716.61 159.02] \
    [list n_9693 {210.09 201.74 210.47 202.02} 210.28 201.88 209.72 g14630__8246/Q 207.01 201.62] \
    [list n_9697 {663.13 192.78 663.51 193.06} 663.32 192.92 662.76 g14626__1617/Q 660.05 192.66] \
    [list n_9677 {1666.09 201.74 1666.47 202.02} 1666.28 201.88 1666.84 g14646__2398/Q 1669.55 201.62] \
    [list n_9721 {1792.65 212.38 1793.03 212.66} 1792.84 212.52 1792.28 g14602__8246/Q 1789.57 212.78] \
    [list n_9706 {1826.81 212.38 1827.19 212.66} 1827.00 212.52 1827.56 g14617__5477/Q 1830.27 212.78]]
...
foreach contract $patch_contract {
    lassign $contract net expected_box start_x start_y end_x source_q source_q_x source_q_y
    ...
    if {$marker_status eq "PASS" && $via_status eq "PASS" &&
        ...
        $inside_status eq "PASS" && [lp_close [expr {abs($end_x - $start_x)}] 0.56]} {
    ...
    puts $contract_fh "$net\t[join $expected_box { }]\t[format %.2f $start_x]\t[format %.2f $start_y]\t[format %.2f $end_x]\t[format %.2f $start_y]\t0.56\t0.28\t$source_q\t[format {%.2f %.2f} $source_q_x $source_q_y]\t$marker_status\t$via_status\t$met2_status\t$source_status\t$inside_status\t$contract_status"
}
close $contract_fh
...
puts $command_fh "POLICY=EXACT_SIX_NET_ONE_GRID_MET1_WIRE_EDITOR_EXTENSIONS"
...
if {!$numeric_post ||
    ...
    set status(RESULT) SIX_MET1_LANDING_EXTENSIONS_DRC_ZERO_VALIDATED
} elseif {$post_drc_count == 6 && $post_marker_count == 6 &&
        [llength $post_rows] == 6 && $post_nets eq $expected_nets &&
        $post_database_total == 27} {
    set status(RESULT) SIX_MET1_LANDING_EXTENSIONS_NO_IMPROVEMENT
} else {
    set status(RESULT) SIX_MET1_LANDING_EXTENSIONS_CHANGED_NOT_CLOSED
}
