...
- [VERIFIED] Server step-7 block run for `ddr16_pairer` used commit `bb6a8b8cd66e4954efced5cfbfd20c27c213e608`. Xcelium `tb_spadmic_ddr16_tx_pairer_unit` passed with `14 pass / 0 fail` and `TB_DDR16_PAIRER_RC=0`; Genus run `genus_ooc_ddr16_pairer_20260709_0705` passed one block with `tool_error=0`, `unresolved=0`, `inferred_latch=0`, `no_clock_waveform=0`, total area `5685.942 um^2`, and worst shown relaxed setup slack `+3496 ps`; Innovus OOC gate `innovus_ooc_ddr16_pairer_20260709_0706` reported `READY_FOR_NEXT_IMPORT_TEMPLATE` with `PNR_RC=0`. This remains non-signoff OOC collateral, not routed Innovus implementation.
- [IMPLEMENTED] `TOP/syn/constraints/ooc/spadmic_ddrs2_adapter.sdc` now creates a local `clk_160m_i` OOC clock and basic IO delays for DDRs2 adapter feasibility runs; this is still typical-only and not DDRs2 macro signoff.
- [VERIFIED] Server step-8 first-pass block run for `ddrs2_adapter` used commit `ed09a90dab7b260dcc4b07cb9dc7350754c399fa`. Xcelium `tb_spadmic_ddrs2_adapter_unit` passed with `17 pass / 0 fail` and `TB_DDRS2_ADAPTER_RC=0`; Genus run `genus_ooc_ddrs2_adapter_20260709_0710` passed one block with `tool_error=0`, `unresolved=0`, `inferred_latch=0`, total area `1103.744 um^2`, and generated netlist/SDC/SDF; Innovus OOC gate `innovus_ooc_ddrs2_adapter_20260709_0710` reported `READY_FOR_NEXT_IMPORT_TEMPLATE` with `PNR_RC=0`. This first pass is not accepted as clocked timing evidence because `report_clocks.rpt` said `No clocks to report`.
- [IMPLEMENTED] The matrix-top Genus OOC wrapper now selects `TOP/syn/constraints/ooc/<top_module>.sdc` or `TOP/syn/constraints/ooc/<block>.sdc` before falling back to `matrix_top_ooc_common.sdc`, writes the selected path to `selected_sdc.txt`, and records it in the run summaries. This fixes the first DDRs2 adapter run where the local `spadmic_ddrs2_adapter.sdc` existed but was not sourced.
- [IMPLEMENTED] Builder added the first real TOP OOC hardening flow for `ddr16_pairer`: `TOP/pnr/scripts/gen_ooc_block_harden_plan.py`, `TOP/pnr/scripts/run_innovus_ooc_harden_block.sh`, and `TOP/pnr/scripts/run_innovus_ooc_harden_block.tcl`. The flow uses the read-only SPADMIC2 layout audit CSVs to generate a local abstract pin/floorplan plan, constrains ordinary signals to `MET1`-`MET3`, creates one north `VDD` and one north `VSS` access pin on `METTP`, runs place/CTS/route/filler/post-route setup/hold/DRV/Innovus DRC/connectivity, and exports DEF/LEF/GDS/routed-netlist handoff collateral. Completion is `ABSTRACT_READY_FOR_TOP_REVIEW` only, not `SIGNOFF_READY`; PVS, PEX, MMMC, foundry LVS, and direct OA import remain separate gates.
- [IMPLEMENTED] OOC hardening now treats local PG special routing as an explicit option, not a default acceptance gate. The default abstract mode exports `METTP` `VDD`/`VSS` access pins and labels PG connectivity `DEFERRED_TOP_LEVEL_HOOKUP`; independent Innovus DRC and regular connectivity must still pass before `ABSTRACT_READY_FOR_TOP_REVIEW`. Use `SPADMIC_OOC_ENABLE_PG_SROUTE=1` only for experimental local PG special-route runs.
- [IMPLEMENTED] The OOC hardening wrapper now forces DRC-safe filler mode by default (`setFillerMode -add_fillers_with_drc false` variants), records `FILLER_MODE.rpt`, runs bounded post-route ECO cleanup (`ecoRoute -target`, `ecoRoute -fix_drc`, `ecoRoute`), and ignores only the expected `VDD`/`VSS` no-route messages in regular connectivity when PG is intentionally deferred to top-level hookup.
- [IMPLEMENTED] TX OOC hardening now has an aggressive `SPADMIC_OOC_ROUTE_PROFILE=met2_first_antenna` mode for the wide DDRs2-aligned `tx_egress_core`: ordinary signal routing is constrained to MET2-MET3, route effort and antenna repair are enabled, final marker classification is reported separately, and unresolved non-PG markers block `ABSTRACT_READY_FOR_TOP_REVIEW`. Bounded fallback height overrides are available through `SPADMIC_OOC_CORE_HEIGHT_UM=160` and then `170`.
- [IMPLEMENTED] The preferred TX physical recovery path is now leaf-first: harden `ddrs2_adapter`, `ddr16_pairer`, `output_fifo`, and `event_bundle_tx` as separate OOC abstracts, then close a local `tx_egress_assembly` review. `spadmic_tx_egress_core` keeps its external interface but now instantiates the four leaves directly so the RTL hierarchy matches the physical split. The wide monolithic `tx_egress_core` route-profile loop remains a regression/fallback, not the primary closure path.
- [DOCUMENTED] Session closeout for the 2026-07-09 matrix-side/TX OOC work is captured in `TOP/docs/30_MATRIX_SIDE_SUBBLOCK_PNR_SESSION_CLOSEOUT_20260709.md`. It records the clean four-leaf abstracts, fixed-leaf assembly/smoke evidence, the split `TX_DDR_STRIP`/`TX_PACKET_CORE` decision, the clean `tx_ddr_strip` abstract, the current `tx_packet_core` marker/debug paths, and the GDS streamout-map re-export gate. This remains typical-only OOC/review evidence, not signoff.
