...
- [VERIFIED] Server step-6 partial block run for `output_fifo` used commit `eccd432b9801f8781c16ef5fa7494eded08ff77c`. Xcelium `tb_spadmic_output_fifo_unit` and `tb_spadmic_output_fifo_ddr_marker_unit` passed with `TB_FIFO_RC=0` and `TB_FIFO_MARKER_RC=0`; Genus run `genus_ooc_output_fifo_20260709_0653` passed one block with top module `spadmic_output_fifo_topcfg`, `tool_error=0`, `unresolved=0`, `inferred_latch=0`, `no_clock_waveform=0`, total area `459392.568 um^2`, and worst shown relaxed setup slack `+542 ps`. Innovus OOC gate for this block remains pending.
